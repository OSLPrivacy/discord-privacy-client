...
- **The docs explicitly couple the lock to plaintext teardown.** `docs/design/external-overlay-security-contract.md:23-26`: plaintext is *"zeroized on eviction, **lock**, context loss, app switch, and drop."* The model forbids the lock touching display.
- **Six separate doc statements say the composer may hide or disable itself**, which the model forbids: `docs/design/external-overlay-security-contract.md:14-16` (*"Moving, resizing, minimizing, changing focus, changing account or chat, or losing geometry certainty hides it immediately"*), `docs/design/osl-gui-final-plan.md:361`, `:363-365` (*"preferred"*, not always), `:467`, `:600`, and `docs/design/osl-social-hub-implementation-plan.md:270`.
...
Declared at `lib.rs:5` (`pub mod external_overlay;`) and referenced from **nowhere else in the tree**. Its whole public surface is unreferenced: `ComposerOverlayGuard` (`:155`), `ComposerOverlayDecision` (`:149`), `OverlayHiddenReason` (`:136`), `DecryptionOverlayGuard` (`:241`), `DecryptedHitTarget` (`:233`), `EncryptedCarrierBinding` (`:215`), `VisiblePlaintextCache` (`:366`), `EncryptedLocalOverlayCacheRecord` (`:484`), `ComposerCalibration` (`:117`), and more.

**Classification: abandoned — superseded by `native_discord_overlay.rs` + `native_discord_adapter.rs`.** This module is the literal implementation of `docs/design/external-overlay-security-contract.md`, which itself admits at `:10-11` that *"The external overlay is not enabled for any service yet"*.
