...
- ✅ **J6 · Shared memory/wiki/compact reports/trap ledger** — versioned compact memory-card rules,
  ...
  account-by-account rollout evidence. J24 acceptance guard: **Keep compact reports and trap-ledger rules current after each wave.**
  A wave is not closed until its compact report says what changed, what evidence was run, what
  remains blocked, and whether the trap ledger was updated, left unchanged because no durable trap
  changed, or intentionally pruned.
  `weight: 2` `earned: 2`
