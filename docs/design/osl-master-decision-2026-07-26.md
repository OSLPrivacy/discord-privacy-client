...
- `r16` — non-scoring D7 safety reconciliation on committed product HEAD
  ...
  Production `Opened` emission and admission do not consult locked mutual consent; the existing
  signed scope-bound consent contract is `implemented-unwired`. Immediate safe state is suppression
  of outbound `Opened` plus rejection of inbound `Opened` before ledger/UI while preserving
  `Received`; restoration requires durable scope/identity-bound, signed, expiring, revocable mutual
  consent checked under the receipt-state lock at both boundaries. Exact website candidate
  `4e2256333c53e6b6e17462657260f5d6499ec9ee` is clean and local-gate green with checkout disabled,
  but remains unpushed/undeployed and lacks Pages/live SHA and keyserver-redemption proof; H1 stays
  3/4. Score remains **97 / 303**.
...
- `r12` — truth-lane reachability correction. Four hand-checked zero-production-caller findings
  ...
  **INTERNAL ONLY**; the Notes cluster was exposed by one present-tense website terms sentence,
  which is now withdrawn and leaves the app honestly saying `available: false`; the burn-alert
  signature layer remains **SOLD** because `README.md:85` says a signed notice is sent even though
  that layer has no production caller. This does not erase the newer, separate authenticated
  `0x0A` revocation path; it means the public word “signed” and the whole peer action still outrun
  runtime proof. The website's pricing/status/string gates all passed while the Notes and peer-Burn
  leaks were present, proving the structural limit: a truthful-looking string gate cannot establish
  call-path reachability. The site keeps its pre-launch/v1 framing; the FAQ now labels peer Burn
  `Planned`, and Pro-expiry terms no longer imply Notes already works. No acceptance point is
  awarded or withdrawn: the existing claim-tool rows cover registry/string consistency, not a Rust
  call graph, and pretending otherwise would repeat the false-confidence defect.
...
| Website | exact `4e2256333c53e6b6e17462657260f5d6499ec9ee` is clean local `test-proven-only`; H1 +0 | The candidate closes the earlier local build-artifact contract gaps and passes its local gates. Checkout is deliberately disabled because paid-code redemption/one-month enforcement is absent. It is four commits ahead of the local `origin/main`, unpushed and undeployed; Pages dashboard/build evidence, a public SHA-bound `/build.json`, and keyserver redemption remain absent/`unknown`. No production reachability or H1 point follows from local verification. |
