...
| Scope/message burn and TTL | `crates/ipc/src/{commands,burned_scopes_file,scope_ttl_file,scope_blobs_file}.rs` | `needs platform adapter` | Burn is local and server-side STATE deletion plus a cooperative peer request, not cryptographic erasure and not native deletion. Some GC-per-user and server-wide local deletion paths remain unimplemented. |
