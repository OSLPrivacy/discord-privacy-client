...
|---|---|---|---|
| Group sender keys | `implemented-unwired` — implemented and enabled in the IPC core, but unreachable in the shipping app | `crates/ipc/src/state.rs:413` defaults `sender_keys_enabled` to true; the v5 router accepts group scopes at `crates/ipc/src/commands.rs:4523-4543`; every production `HubConversationContext` in `apps/osl-hub/src/broker.rs:303,348,1964` is a DM, and the manual-peer path forces `ScopeKind::Dm` at `:329-336`. Sender attribution is enforced only by `verify_v5_sender_discord_binding` in `crates/ipc/src/commands.rs:9401-9426`: it trusts the carrier-reported author and that account's TOFU pin. T5 P0 must repair the verification ceremony before this is a dependable group-authentication boundary. | "Group chats currently get the same protection as direct messages. The group-specific scheme (sender keys) is implemented but is not reachable in the app, because the app does not yet have group conversations. When it ships it will bound how far back a leaked key reads — it will not make one group member unable to impersonate another, and it will not make a removed member unable to read messages sent before the next key rotation. Sender attribution depends on the carrier reporting the author and on that account's TOFU pin." |
...
| Word-bank cover text | `implemented-unwired` → `Planned` | The deterministic bigram codec renders capability tokens as words (`crates/stego/src/bigram.rs`); the v1 send path currently coerces Mode 1 to a visible `DPC0::` capsule (`crates/ipc/src/commands.rs:2640-2660`). | "Planned. OSL is working on cover text that reads like ordinary chat instead of an obvious encrypted blob." It uses a bounded vocabulary and follows protected-message timing, so it is not statistically undetectable and does not hide who communicates, when, or how often. |
| AI-selected cover text | `implemented-unwired` → `Planned` | `crates/cover-ai/src/candidates.rs` draws independent capabilities and `scorer.rs` only ranks already-rendered covers; neither has a production send consumer. `crates/cover-ai/tests/selection_gain.rs` fixes the measured selection knee at K=13. | "Planned. A local model may choose among already-valid covers; it does not create encryption, change message bytes, or authorize sending." Selection deliberately changes the candidate distribution, and a dedicated steganalyser or a reader comparing it with a person's normal writing may still identify machine-written cover. |
| Cloud carrier generation | `designed-only` → `Planned`, **not offered in v1** | `docs/design/ai-carrier-architecture.md` records D72's v1 decision: `cloudGenerationOffered: false`; no cloud credits or checkout are offered. | "Not offered in v1. If a future opt-in cloud option is offered, it only ever sees text the platform already has." It must never be called end-to-end encrypted; request IP, timing, frequency, size, and the cover conversation used can still be metadata visible to OSL. |
| Local sensitive-content warning | `implemented-unwired` → `Planned` | The existing detector runs in process with no HTTP/model dependency or I/O (`apps/osl-hub/src/privacy_scan.rs:1-6`); `crates/sensitive-classify/src/categories.rs` maps only structured secret/identity categories to a warning and drops political/lexical categories. | "Planned. OSL may show a local, non-blocking warning before an unencrypted send when it detects a credential or other structured sensitive data." It is not moderation, never judges the message, never blocks sending, does not inspect images in v1, and ordinary political discussion must not trigger it. |
| OSL Spaces | `designed-only` → `Planned` | The shipping Servers page says only that shared encrypted spaces will appear when sender, membership, delivery, and history security are complete (`apps/osl-hub-ui/src/main.ts:4188-4195`). Its listed Discord, Telegram, Signal, and Snapchat group surfaces are all marked "Not available yet"; there is no OSL Space implementation. | "OSL Spaces are planned. They are not in the current release." Do not add a feature badge, checkout claim, or capability detail until its exact capability has shipped and earned its own evidence row. **Ownership:** these are the Spaces rows; T14-F3 is Circles-only. |
...
| **"OSL verifies you own your Discord account" / "Discord account ownership verified" / "your Discord account is verified"** | The account-binding endpoints do not contact Discord or require OAuth. They mint a challenge for any claimed snowflake and accept a signature from the claimant's own registered OSL identity key. A recorded binding means only that an OSL identity claimed a snowflake freshly and once; it does not establish control of that Discord account. |
...
| **"Undetectable" / "statistically undetectable" / "indistinguishable from your normal messages"** applied to cover text | Cover text may be less conspicuous than a visible encrypted capsule, but it follows protected-message timing, has a bounded vocabulary, and can be detected by a dedicated steganalyser or by comparison with a person's normal writing. Do not turn a measurement or a design goal into a detection-evasion guarantee. |
