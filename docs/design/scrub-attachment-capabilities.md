...
Scrub attachment parsing is local-only. Enabled built-in routes select handlers
from bytes, expand archives only in memory, never execute attachment content,
and apply shared expansion, entry-count, recursion, and extracted-text caps.
This is a bounded application-level parser boundary, not an OS-level sandbox.
...
PDF and XLSX are detected but deliberately reported as uninspected. Their
current libraries do not provide hard wall-clock and memory isolation against
hostile files, so OSL will not parse them in the desktop process. They remain
disabled until they can run in a separately bounded worker.
