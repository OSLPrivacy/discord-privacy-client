const STORAGE_KEY = "osl-chats-lab-v5";
...
const icons = {
  ...
  hash: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 3 8 21M16 3l-2 18M4 9h16M3 15h16"/></svg>`,
  spark: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2v5M12 17v5M4.9 4.9l3.5 3.5M15.6 15.6l3.5 3.5M2 12h5M17 12h5M4.9 19.1l3.5-3.5M15.6 8.4l3.5-3.5"/><circle cx="12" cy="12" r="3"/></svg>`,
  bell: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 9a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg>`,
  volume: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 10v4h4l5 4V6l-5 4H5ZM18 9c1.3 1.7 1.3 4.3 0 6M20.5 6.5c3 3 3 8 0 11"/></svg>`,
  volumeOff: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 10v4h4l5 4V6l-5 4H5ZM18 10l4 4M22 10l-4 4"/></svg>`,
...
const seed = {
  ...
  whiteboards: {},
  people: {
    nova: { id: "nova", name: "Nova Reyes", username: "nova", initials: "NR", status: "online", verified: true, color: "linear-gradient(135deg,#fd7096,#8d6cff)", note: "Making an ambient game soundtrack", devices: 3 },
    theo: { id: "theo", name: "Theo Lin", username: "theo", initials: "TL", status: "away", verified: true, color: "linear-gradient(135deg,#f0b557,#ea6f57)", note: "At the studio", devices: 2 },
    maya: { id: "maya", name: "Maya Okafor", username: "maya", initials: "MO", status: "online", verified: true, color: "linear-gradient(135deg,#34c79b,#267dd6)", note: "Deep work until 3", devices: 4 },
    elio: { id: "elio", name: "Elio Park", username: "elio", initials: "EP", status: "offline", verified: false, color: "linear-gradient(135deg,#78869b,#4c5c72)", note: "Offline", devices: 1 },
...
function loadState() {
  try {
    ...
    return parsed?.version === 5 ? parsed : cloneSeed();
...
function activeThreadId() { return state.route === "circles" ? state.activeCircle : state.activeConversation; }
function activeConversation() { return getConversation(activeThreadId()); }
...
function accountDockMarkup() {
  return `<div class="account-dock"><button class="account-identity" data-settings-open="profile" aria-label="Open profile settings">${avatarMarkup({...state.profile,color:state.profile.profileColor})}<span><strong>${escapeHtml(state.profile.username)}</strong><small>${state.profile.hiddenUntilFriend?"Hidden profile":"OSL Chat"}</small></span></button><button class="icon-button" data-settings-open="notifications" title="Notifications" aria-label="Notification settings">${icons.bell}</button><button class="icon-button" data-settings-open="appearance" title="Settings" aria-label="Settings">${icons.settings}</button></div>`;
}
...
function render() {
  applyAppearance();
  const content = state.route === "chats" ? renderChats() : state.route === "circles" ? renderCircles() : renderSettings();
  app.innerHTML = `<div class="app-shell">${content}</div>`;
  bindEvents();
}
...
function conversationListMarkup() {
  ...
  const items = state.conversations.filter((item) => {
    const matchesType = item.type === state.filter;
  ...
  return `<aside class="conversation-list" aria-label="Conversations">
    <div class="list-header">
      <div class="list-tools"><div class="list-primary-nav"><button class="osl-home-button" data-action="osl-home" title="OSL Home" aria-label="OSL Home"><img src="./logo-mark.svg" alt="" /></button><div class="type-tabs" role="tablist"><button class="type-tab ${state.filter==="dm"?"active":""}" data-filter="dm" role="tab" title="Direct messages" aria-label="Direct messages" aria-selected="${state.filter==="dm"}">${icons.chats}</button><button class="type-tab ${state.filter==="circle"?"active":""}" data-filter="circle" role="tab" title="Circles" aria-label="Circles" aria-selected="${state.filter==="circle"}">${icons.circles}</button></div></div><button class="icon-button" data-action="new-chat" title="New chat" aria-label="New chat">${icons.plus}</button></div>
      <div class="search-box">${icons.search}<input id="chat-search" value="${escapeHtml(state.query)}" placeholder="Search" aria-label="Search conversations" /></div>
    </div>
    <div class="conversation-scroll">
      ${items.map((item)=>`<button class="conversation-item ${state.activeConversation===item.id?"active":""}" data-conversation="${item.id}">
        ${conversationAvatar(item)}<span class="conversation-copy"><span class="conversation-line"><span class="conversation-name">${escapeHtml(item.name)}</span></span><span class="conversation-preview">${escapeHtml(item.preview)}</span></span>
        <span class="conversation-kind" title="${state.notifications.mutedConversations.includes(item.id)?"Muted":item.type==="circle"?"Circle":"Direct message"}" aria-label="${state.notifications.mutedConversations.includes(item.id)?"Muted":item.type==="circle"?"Circle":"Direct message"}">${state.notifications.mutedConversations.includes(item.id)?icons.volumeOff:item.type==="circle"?icons.circles:icons.chats}</span>
...
function messageMarkup(message, allMessages) {
  ...
  const viewOnce = message.imageUrl ? `<button class="view-once-photo" data-action="open-once-photo" data-message="${message.id}" aria-label="Open view-once photo"><img src="${escapeHtml(message.imageUrl)}" alt="View-once photo preview" /><span>${icons.image}${message.opened?"Opened":"View once"}</span></button>` : message.viewOnce ? `<button class="view-once" data-action="open-once" data-message="${message.id}">${icons.image} View once · ${escapeHtml(message.viewOnce)}</button>` : "";
  return `<article class="message-group ${mine?"mine":""}" data-message-id="${message.id}">
    ...
    <div class="message-column">
      ...
      ${message.attachment?`<div class="attachment-card"><span class="file-icon">${escapeHtml(message.attachment.type)}</span><span><strong>${escapeHtml(message.attachment.name)}</strong><span>${escapeHtml(message.attachment.size)} · ${escapeHtml(message.attachment.state)}</span></span><button class="download-button" data-action="${message.attachment.type==="CANVAS"?"open-shared-canvas":"mock-download"}" data-board="${escapeHtml(message.attachment.boardId??"")}" aria-label="${message.attachment.type==="CANVAS"?"Open shared canvas":"Save local copy"}">${message.attachment.type==="CANVAS"?icons.palette:icons.download}</button></div>`:""}
...
function renderChats() {
  if (state.chatHome) return `<main id="main-content" class="workspace chat-layout">${conversationListMarkup()}<section class="chat-home-panel"><span>${icons.chats}</span><h1>Chats</h1><div><button class="icon-button" data-action="new-chat" title="New chat" aria-label="New chat">${icons.plus}</button><button class="icon-button" data-action="new-group" title="New group DM" aria-label="New group DM">${icons.users}</button></div></section></main>`;
  ...
  const channels = circle ? `<div class="channel-bar" aria-label="Circle channels">${circle.channels.map((channel)=>`<button class="channel-tab ${state.activeChannel===channel.id?"active":""}" data-channel="${channel.id}"># ${escapeHtml(channel.name)}</button>`).join("")}<button class="channel-icon" data-action="whiteboard" title="Canvas" aria-label="Canvas">${icons.palette}</button><button class="channel-icon" data-action="call" title="Voice" aria-label="Voice">${icons.phone}</button></div>` : "";
  return `<main id="main-content" class="workspace chat-layout ${state.mobileChatOpen?"mobile-chat-open":""}">${conversationListMarkup()}
    <section class="chat-panel" aria-label="${escapeHtml(conversation.name)} chat">
      <div><header class="chat-header"><div class="chat-person"><button class="icon-button chat-home" data-action="chat-home" title="Chats home" aria-label="Chats home">${icons.home}</button>${conversationAvatar(conversation)}<div><h2>${escapeHtml(conversation.name)}</h2><div class="presence">${circle?`${circle.online} online · ${circle.members} members`:`<strong>${person.status}</strong> · ${escapeHtml(person.note)}`}</div></div></div><div class="header-actions"><button class="icon-button" data-action="whiteboard" data-board-mode="shared" title="Shared canvas" aria-label="Shared canvas">${icons.palette}</button><button class="icon-button optional-action" data-action="call" aria-label="Voice call">${icons.phone}</button><button class="icon-button optional-action" data-action="video-call" aria-label="Video call">${icons.video}</button><button class="icon-button security-button" data-action="security" aria-label="Encryption details">${icons.lock}<span>E2EE</span></button></div></header>${channels}</div>
      <div class="message-scroll" id="message-scroll"><div class="encryption-notice" title="End-to-end encrypted">${icons.lock}</div>${messages.map((message)=>messageMarkup(message,messages)).join("")}</div>
      <div class="composer-wrap">${state.replyTo?`<div class="replying-banner visible"><span>Reply</span><button data-action="cancel-reply" aria-label="Cancel reply">×</button></div>`:""}<form class="composer" id="message-form"><div class="composer-start"><button type="button" class="composer-button" data-action="composer-menu" title="More" aria-label="More message tools">${icons.plus}</button><button type="button" class="composer-button secondary-tool" data-action="choose-view-once" title="View-once photo" aria-label="Send view-once photo">${icons.image}</button></div><textarea id="message-input" placeholder="Message ${escapeHtml(conversation.name)}" aria-label="Message"></textarea><div class="composer-end"><button type="button" class="composer-button secondary-tool" data-action="timer" title="Timer" aria-label="Message timer">${icons.clock}</button><button type="button" class="composer-button secondary-tool" data-action="whiteboard" data-board-mode="private" title="Private canvas" aria-label="Draw privately, then send">${icons.palette}</button><button class="send-button" aria-label="Send">${icons.send}</button></div></form></div>
    </section>
  </main>`;
}

function renderCircles() {
  const circle = state.circles[state.activeCircle] ?? Object.values(state.circles)[0];
  const active = circle.channels.find((channel)=>channel.id===state.activeChannel) ?? circle.channels[0];
  const voice = circle.channels.filter((channel)=>channel.id.includes("voice") || channel.name.includes("voice"));
  const textChannels = circle.channels.filter((channel)=>!voice.includes(channel));
  const messages = state.messages[circle.id] ?? [];
  return `<main id="main-content" class="workspace circles-workspace"><aside class="circle-index"><div class="circle-index-head"><span>${icons.circles}</span><button class="icon-button" data-action="new-circle" title="New Circle" aria-label="New Circle">${icons.plus}</button></div>${Object.values(state.circles).map((item)=>`<button class="circle-index-row ${item.id===circle.id?"active":""}" data-circle-select="${item.id}">${conversationAvatar({...item,type:"circle"})}<span>${escapeHtml(item.name)}</span></button>`).join("")}${accountDockMarkup()}</aside>
  <section class="circle-focus"><header class="circle-focus-head"><div><button class="icon-button" data-route="chats" title="Chats home" aria-label="Chats home">${icons.home}</button>${conversationAvatar({...circle,type:"circle"})}<span><h1>${escapeHtml(circle.name)}</h1><small>${circle.online} / ${circle.members}</small></span></div><div class="circle-tools"><button class="icon-button" data-action="new-group" title="New group DM" aria-label="New group DM">${icons.users}</button><button class="icon-button" data-action="video-call" title="Stage" aria-label="Stage">${icons.video}</button><button class="icon-button" data-action="whiteboard" data-board-mode="shared" title="Shared canvas" aria-label="Shared canvas">${icons.palette}</button><button class="icon-button" data-action="security" title="Roles" aria-label="Roles">${icons.lock}</button></div></header>
  <div class="circle-focus-body"><nav class="circle-channels" aria-label="Channels"><div class="channel-section-head"><span>Text</span><button data-action="new-circle" aria-label="New text channel">${icons.plus}</button></div>${textChannels.map((channel)=>`<button class="circle-channel ${active.id===channel.id?"active":""}" data-circle-channel="${channel.id}">${icons.hash}<span>${escapeHtml(channel.name)}</span></button>`).join("")}<div class="channel-section-head"><span>Voice</span><button data-action="call" aria-label="New voice room">${icons.plus}</button></div>${voice.map((channel)=>`<button class="circle-channel voice-channel" data-action="call">${icons.volume}<span>${escapeHtml(channel.name)}</span></button>`).join("")}<button class="circle-channel voice-channel" data-action="call">${icons.volume}<span>studio</span></button></nav>
  <section class="circle-chat"><header class="circle-chat-head"><span>${icons.hash}</span><strong>${escapeHtml(active.name)}</strong><button class="icon-button" data-action="call" title="Voice" aria-label="Voice call">${icons.phone}</button><button class="icon-button" data-action="whiteboard" data-board-mode="shared" title="Canvas" aria-label="Shared canvas">${icons.palette}</button></header><div class="message-scroll" id="message-scroll">${messages.map((message)=>messageMarkup(message,messages)).join("")}</div><div class="composer-wrap"><form class="composer" id="message-form"><div class="composer-start"><button type="button" class="composer-button" data-action="composer-menu" aria-label="More message tools">${icons.plus}</button><button type="button" class="composer-button" data-action="choose-view-once" title="View-once photo" aria-label="Send view-once photo">${icons.image}</button></div><textarea id="message-input" placeholder="Message #${escapeHtml(active.name)}" aria-label="Message"></textarea><div class="composer-end"><button type="button" class="composer-button" data-action="whiteboard" data-board-mode="private" title="Private canvas" aria-label="Private canvas">${icons.palette}</button><button class="send-button" aria-label="Send">${icons.send}</button></div></form></div></section></div></section></main>`;
}
...
function renderDeveloper() {
  ...
  return `<main id="main-content" class="workspace content-layout"><div class="page"><div class="page-shell"><header class="page-header"><div><span class="eyebrow">OSL Chats</span><h1>Developer</h1><p>Bots, automations and tools. Local by default; every capability is visible and revocable.</p></div><button class="primary-button" data-action="create-bot">New project</button></header>
...
function renderData() {
  return `<main id="main-content" class="workspace content-layout"><div class="page"><div class="page-shell"><header class="page-header"><div><span class="eyebrow">OSL Chats</span><h1>Data</h1><p>Local storage, temporary delivery and portable exports.</p></div><button class="primary-button" data-export="archive">Download everything</button></header>
...
function renderCalmSettings() {
  const tabs = [["appearance",icons.palette,"Appearance"],["profile",icons.users,"Profile"],["notifications",icons.bell,"Notifications"],["developer",icons.code,"Developer"],["automations",icons.spark,"Automations"]];
  const themes = [["midnight","OSL","#0a0a0a"],["aurora","Aurora","#071814"],["orchid","Orchid","#160d22"],["paper","Paper","#edf1f4"]];
  let panel = "";
  if (state.settingsTab === "profile") panel = `<section class="settings-panel"><div class="profile-preview" style="--profile:${state.profile.profileColor};--banner:${state.profile.bannerColor}"><div class="profile-banner"></div><div class="profile-preview-body">${avatarMarkup({...state.profile,color:state.profile.profileColor})}<strong>${escapeHtml(state.profile.username)}</strong><small>${state.profile.hiddenUntilFriend?"Hidden until accepted":escapeHtml(state.profile.status)}</small></div></div><div class="settings-title"><span>${icons.users}</span><h1>Profile</h1></div><div class="form-grid"><label class="field full"><span>Username</span><input id="profile-handle" value="${escapeHtml(state.profile.username)}" autocomplete="username" /></label><label class="field full"><span>Status</span><input id="profile-status" value="${escapeHtml(state.profile.status)}" /></label><label class="field full"><span>Avatar or GIF URL</span><input id="avatar-url" value="${escapeHtml(state.profile.avatarUrl)}" placeholder="URL" /></label><label class="field"><span>Profile color</span><input id="profile-color" type="color" value="${state.profile.profileColor}" /></label><label class="field"><span>Banner color</span><input id="banner-color" type="color" value="${state.profile.bannerColor}" /></label><label class="field full"><span>Profile effect</span><select id="profile-effect"><option value="none">None</option><option value="scan" ${state.profile.profileEffect==="scan"?"selected":""}>Cyan scan</option><option value="stars" ${state.profile.profileEffect==="stars"?"selected":""}>Quiet stars</option><option value="grain" ${state.profile.profileEffect==="grain"?"selected":""}>Film grain</option></select></label></div><div class="setting-row"><span>Hidden until accepted</span><button class="toggle ${state.profile.hiddenUntilFriend?"on":""}" data-action="toggle-hidden-profile" aria-label="Toggle hidden profile"></button></div><div class="button-row"><button class="secondary-button" data-action="choose-avatar">Upload image / GIF</button><button class="primary-button" data-action="save-profile">Save</button></div><div class="profile-free-grid"><button data-action="custom-css">Frame</button><button data-action="custom-css">Badge</button><button data-action="custom-css">Nameplate</button><button data-action="custom-css">Theme</button><button data-action="custom-css">Sound</button><button data-action="custom-css">CSS</button></div></section>`;
  else if (state.settingsTab === "notifications") {
    const groups = [
      [["enabled","Notifications"],["badge","Badge"],["sound","Sound"],["quietHours","Quiet hours"]],
      [["directMessages","Direct messages"],["circles","Circles"],["calls","Calls"],["mentions","Mentions"],["replies","Replies"],["reactions","Reactions"]],
      [["friendRequests","Friend requests"],["security","Security changes"],["messagePreview","Message previews"]],
    ];
    panel = `<section class="settings-panel"><div class="settings-title"><span>${icons.bell}</span><h1>Notifications</h1></div>${groups.map((rows,index)=>`<div class="settings-list notification-group" aria-label="${index===0?"General":index===1?"Messages and calls":"Privacy and security"}">${rows.map(([key,label])=>`<div class="settings-row"><span>${key==="sound"?icons.volume:key==="quietHours"?icons.clock:icons.bell}</span><span><strong>${label}</strong></span><button class="toggle ${state.notifications[key]?"on":""}" data-notification="${key}" aria-label="Toggle ${label}"></button></div>`).join("")}</div>`).join("")}<div class="settings-title settings-gap"><span>${icons.volumeOff}</span><h1>Muted</h1></div><div class="settings-list">${state.notifications.mutedConversations.map(id=>{const item=getConversation(id);return `<div class="settings-row"><span>${icons.volumeOff}</span><span><strong>${escapeHtml(item.name)}</strong></span><button class="secondary-button" data-unmute="${id}">Unmute</button></div>`}).join("")||`<div class="settings-empty">Right-click a person or Circle to mute it.</div>`}</div></section>`;
  }
  else if (state.settingsTab === "developer") panel = `<section class="settings-panel"><div class="settings-title"><span>${icons.code}</span><h1>Developer</h1><button class="icon-button" data-action="create-bot" title="New project" aria-label="New project">${icons.plus}</button></div><div class="settings-list">${state.bots.map((bot)=>`<div class="settings-row"><span class="bot-dot" style="--bot:${bot.accent}"></span><span><strong>${escapeHtml(bot.name)}</strong><small>${bot.events} events · ${bot.permissions.length} grants</small></span><button class="toggle ${bot.status==="running"?"on":""}" data-bot-toggle="${bot.id}" aria-label="Toggle ${escapeHtml(bot.name)}"></button></div>`).join("")}<button class="settings-row action-row" data-action="emit-event">${icons.terminal}<span><strong>Event stream</strong><small>${state.events[0]?.type ?? "Ready"}</small></span></button><button class="settings-row action-row" data-action="copy-code">${icons.code}<span><strong>SDK</strong><small>Commands · panels · calls · canvas</small></span></button><button class="settings-row action-row" data-export="archive">${icons.download}<span><strong>Export</strong><small>Portable archive</small></span></button></div><pre class="code settings-code" id="sdk-code">defineProject({ permissions: ["messages.write:selected"] })</pre></section>`;
  else if (state.settingsTab === "automations") panel = `<section class="settings-panel"><div class="settings-title"><span>${icons.spark}</span><h1>Automations</h1><button class="icon-button" data-action="new-automation" title="New automation" aria-label="New automation">${icons.plus}</button></div><div class="settings-list">${state.automations.map((item)=>`<div class="settings-row"><span class="bot-dot" style="--bot:${item.color}"></span><span><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.trigger)}</small></span><button class="toggle ${item.enabled?"on":""}" data-automation-toggle="${item.id}" aria-label="Toggle ${escapeHtml(item.name)}"></button></div>`).join("")}</div></section>`;
  else panel = `<section class="settings-panel"><div class="settings-title"><span>${icons.palette}</span><h1>Appearance</h1></div><div class="theme-grid">${themes.map(([id,label,color])=>`<button class="theme-card ${state.appearance.theme===id?"active":""}" data-theme-choice="${id}" style="--preview:${color}"><span>${label}</span></button>`).join("")}</div><div class="color-row settings-gap">${["#06b6d4","#8da2ff","#e979b5","#f4bd62","#f07b72","#9be062"].map(color=>`<button class="color-swatch ${state.appearance.accent===color?"active":""}" data-accent="${color}" style="--swatch:${color}" aria-label="Use ${color}"></button>`).join("")}<label class="custom-color" title="Custom accent"><input id="custom-accent" type="color" value="${state.appearance.accent}" aria-label="Custom accent color" /></label></div><div class="form-grid settings-gap"><label class="field"><span>Density</span><select id="density"><option value="compact" ${state.appearance.density==="compact"?"selected":""}>Compact</option><option value="comfortable" ${state.appearance.density==="comfortable"?"selected":""}>Balanced</option><option value="airy" ${state.appearance.density==="airy"?"selected":""}>Airy</option></select></label><label class="field"><span>Messages</span><select id="message-shape"><option value="compact">Precise</option><option value="rounded" ${state.appearance.messageShape==="rounded"?"selected":""}>Rounded</option><option value="soft" ${state.appearance.messageShape==="soft"?"selected":""}>Soft</option></select></label><label class="field full"><span>Scale</span><input id="font-scale" type="range" min="0.85" max="1.25" step="0.05" value="${state.appearance.fontScale}" /></label></div><div class="profile-free-grid settings-gap"><button data-action="custom-css">Chat themes</button><button data-action="custom-css">Fonts</button><button data-action="custom-css">Icons</button><button data-action="custom-css">Motion</button><button data-action="custom-css">Sounds</button><button data-action="custom-css">CSS</button></div></section>`;
  return `<main id="main-content" class="workspace settings-workspace"><nav class="settings-nav" aria-label="Settings"><button class="settings-tab" data-route="chats" title="Chats" aria-label="Back to Chats">${icons.home}</button><i></i>${tabs.map(([id,icon,label])=>`<button class="settings-tab ${state.settingsTab===id?"active":""}" data-settings-tab="${id}" title="${label}" aria-label="${label}">${icon}</button>`).join("")}<span></span><button class="settings-tab" data-action="reset" title="Reset Lab" aria-label="Reset Lab">${icons.archive}</button></nav><div class="settings-stage">${panel}</div></main>`;
}

function renderSettings() {
  return renderCalmSettings();
  const themes = [["midnight","OSL","linear-gradient(135deg,#0a0a0a,#141414)"],["aurora","Aurora","linear-gradient(135deg,#071814,#16433c)"],["orchid","Orchid","linear-gradient(135deg,#160d22,#633f72)"],["paper","Paper","linear-gradient(135deg,#edf1f4,#d9e0e6)"]];
  ...
  return `<main id="main-content" class="workspace content-layout"><div class="page"><div class="page-shell"><header class="page-header"><div><span class="eyebrow">OSL Chats</span><h1>Workspace</h1><p>Profile, appearance, behavior and creator controls.</p></div><button class="secondary-button" data-action="reset">Reset lab</button></header>
  ...
  <section class="card full"><div class="card-head"><div><span class="eyebrow">Modules</span><h2>Supported power tools</h2><p>Documented, permissioned and stable.</p></div><span class="badge local">Modular</span></div><div class="capability-matrix">${[
...
function whiteboardModal(mode = "shared") {
  const shared = mode === "shared";
  const existing = shared ? state.whiteboards[activeThreadId()] : "";
  showModal(shared?"Shared canvas":"Private canvas",`<div class="canvas-mode"><button class="${shared?"active":""}" data-action="switch-shared-canvas">Shared</button><button class="${!shared?"active":""}" data-action="switch-private-canvas">Private</button><button data-action="new-shared-canvas">New shared</button></div><div class="canvas-toolbar"><button class="tool-chip active" data-canvas-tool="pen">Pen</button><button class="tool-chip" data-canvas-tool="note">Note</button><button class="tool-chip" data-canvas-tool="frame">Frame</button><input id="canvas-color" type="color" value="${state.appearance.accent}" aria-label="Drawing color" /><button class="tool-chip" data-action="clear-canvas">Clear</button></div><canvas id="whiteboard" width="900" height="440" data-board-mode="${mode}" data-existing="${escapeHtml(existing)}" aria-label="${shared?"Shared":"Private"} whiteboard"></canvas><div class="canvas-presence">${shared?`<span><i style="--presence:#5fe5d2"></i>You</span><span><i style="--presence:#e979b5"></i>Nova</span><span class="badge local">E2EE</span>`:`<span>${icons.lock} Only you can see this until sent</span>`}</div>`,`<button class="secondary-button" data-action="close-modal">Close</button><button class="primary-button" data-action="share-canvas">${shared?"Update":"Send"}</button>`);
  bindCanvas();
}
...
function bindCanvas() {
  ...
  if (canvas.dataset.existing) { const image=new Image();image.onload=()=>context.drawImage(image,0,0,canvas.width,canvas.height);image.src=canvas.dataset.existing; }
  let drawing = false;
...
function friendModal() {
  showModal("Add friend",`<label class="field"><span>Username</span><input id="friend-query" placeholder="nova" autocomplete="off" /></label><div class="button-row"><button class="friend-method"><b>▦</b><strong>QR</strong><span>Scan keys</span></button><button class="friend-method"><b>↗</b><strong>Invite</strong><span>Private link</span></button><button class="friend-method"><b>⌁</b><strong>Nearby</strong><span>Local discovery</span></button></div>`,`<button class="secondary-button" data-action="close-modal">Cancel</button><button class="primary-button" data-action="send-friend-request">Add</button>`);
}

function groupModal() {
  showModal("New group DM",`<label class="field"><span>Name</span><input id="group-name" placeholder="Weekend build" /></label><div class="settings-list modal-note">${["nova","theo","maya","elio"].map(id=>`<label class="settings-row"><span>${avatarMarkup(getPerson(id))}</span><span><strong>${escapeHtml(getPerson(id).name)}</strong><small>${escapeHtml(getPerson(id).username.replace("@osl",""))}</small></span><input type="checkbox" ${id!=="elio"?"checked":""} /></label>`).join("")}</div>`,`<button class="secondary-button" data-action="close-modal">Cancel</button><button class="primary-button" data-action="create-group-confirm">Create</button>`);
}

function composerMenu() {
  showModal("Add",`<div class="composer-menu-grid"><button data-action="attach">${icons.attach}<span>File</span></button><button data-action="choose-view-once">${icons.image}<span>View once</span></button><button data-action="switch-private-canvas">${icons.palette}<span>Private canvas</span></button><button data-action="switch-shared-canvas">${icons.users}<span>Shared canvas</span></button><button data-action="timer">${icons.clock}<span>Timer</span></button><button data-action="call">${icons.phone}<span>Voice</span></button></div>`);
}

function showViewOncePhoto(message) {
  if (message.opened) return toast("This photo was already opened","View once");
  showModal("View once",`<div class="once-viewer"><img src="${escapeHtml(message.imageUrl)}" alt="View-once photo" /><span>Closes when you leave</span></div>`,`<button class="primary-button" data-action="close-once-photo" data-message="${message.id}">Done</button>`);
}

function showConversationMenu(conversation, x, y) {
  document.querySelector(".context-menu")?.remove();
  const muted = state.notifications.mutedConversations.includes(conversation.id);
  const menu = document.createElement("div");menu.className="context-menu";menu.style.left=`${Math.min(x,innerWidth-210)}px`;menu.style.top=`${Math.min(y,innerHeight-150)}px`;
  menu.innerHTML=`<button data-context-mute="${conversation.id}">${muted?icons.volume:icons.volumeOff}<span>${muted?"Unmute":"Mute"}</span></button><button data-context-notifications="${conversation.id}">${icons.bell}<span>Notifications</span></button><button data-context-profile="${conversation.id}">${icons.users}<span>Profile</span></button>`;
...
function bindEvents() {
  ...
  app.querySelectorAll("[data-conversation]").forEach((button)=>{button.addEventListener("click",()=>{const item=getConversation(button.dataset.conversation);if(item.type==="circle"){state.activeCircle=item.id;state.activeChannel="lobby";state.route="circles";}else{state.activeConversation=item.id;state.mobileChatOpen=true;state.chatHome=false;}item.unread=0;save();render();});button.addEventListener("contextmenu",(event)=>{event.preventDefault();showConversationMenu(getConversation(button.dataset.conversation),event.clientX,event.clientY);});});
  ...
  app.querySelectorAll("[data-circle-select]").forEach((button)=>button.addEventListener("click",()=>{state.activeCircle=button.dataset.circleSelect;save();render();}));
  app.querySelectorAll("[data-circle-channel]").forEach((button)=>button.addEventListener("click",()=>{state.activeChannel=button.dataset.circleChannel;save();render();}));
  app.querySelectorAll("[data-settings-tab]").forEach((button)=>button.addEventListener("click",()=>{state.settingsTab=button.dataset.settingsTab;save();render();}));
  app.querySelectorAll("[data-settings-open]").forEach((button)=>button.addEventListener("click",()=>{state.settingsTab=button.dataset.settingsOpen;state.route="settings";save();render();}));
  app.querySelectorAll("[data-notification]").forEach((button)=>button.addEventListener("click",()=>{const key=button.dataset.notification;state.notifications[key]=!state.notifications[key];save();render();}));
  app.querySelectorAll("[data-unmute]").forEach((button)=>button.addEventListener("click",()=>{state.notifications.mutedConversations=state.notifications.mutedConversations.filter(id=>id!==button.dataset.unmute);save();render();}));
  ...
  app.querySelector("#custom-accent")?.addEventListener("input",(event)=>{state.appearance.accent=event.target.value;save();applyAppearance();});
...
function handleAction(action, element) {
  if (action === "osl-home") { window.location.assign("http://127.0.0.1:4177/?v=4"); return; }
  if (action === "new-chat" || action === "friend-card") return friendModal();
  if (action === "new-circle") return circleModal();
  if (action === "new-group") return groupModal();
  if (action === "open-circle") { state.activeConversation=element.dataset.circle||state.activeCircle;state.filter="circle";state.route="chats";state.mobileChatOpen=true;save();render();return; }
  if (action === "create-bot") return projectModal();
  if (action === "whiteboard" || action === "open-shared-canvas") return whiteboardModal(element.dataset.boardMode||"shared");
  if (action === "switch-shared-canvas") { closeModal();return whiteboardModal("shared"); }
  if (action === "switch-private-canvas") { closeModal();return whiteboardModal("private"); }
  if (action === "new-shared-canvas") { delete state.whiteboards[activeThreadId()];save();closeModal();return whiteboardModal("shared"); }
  if (action === "composer-menu") return composerMenu();
  if (action === "choose-view-once") { closeModal();mediaFile.click();return; }
  if (action === "open-once-photo") { const message=state.messages[activeThreadId()]?.find(item=>item.id===element.dataset.message);if(message)return showViewOncePhoto(message); }
  ...
  if (action === "quick-react" || action === "react") { const messages=state.messages[activeThreadId()];const message=messages.find(item=>item.id===element.dataset.message);message.reactions??=[];let reaction=message.reactions.find(item=>item.e===(element.dataset.emoji??"♡"));if(reaction){reaction.mine=!reaction.mine;reaction.n+=reaction.mine?1:-1;}else message.reactions.push({e:"♡",n:1,mine:true});save();render();return; }
  if (action === "scrub-message") { const list=state.messages[activeThreadId()];const message=list.find(item=>item.id===element.dataset.message);message.text="Message scrubbed by its author";delete message.attachment;message.scrubbed=true;addEvent("message.scrubbed",`${activeThreadId()}/${message.id} · author action`);save();render();return; }
  ...
  if (action === "chat-home") { state.chatHome=true;state.mobileChatOpen=false;save();render();return; }
  ...
  if (action === "toggle-hidden-profile") { state.profile.hiddenUntilFriend=!state.profile.hiddenUntilFriend;save();render();return; }
  if (action === "save-profile") { state.profile.username=document.querySelector("#profile-handle").value.trim().replace(/^@|@osl$/g,"")||"liam";state.profile.name=state.profile.username;state.profile.status=document.querySelector("#profile-status").value;state.profile.avatarUrl=document.querySelector("#avatar-url").value;state.profile.profileColor=document.querySelector("#profile-color").value;state.profile.bannerColor=document.querySelector("#banner-color").value;state.profile.profileEffect=document.querySelector("#profile-effect").value;save();render();toast("Profile saved locally");return; }
...
mediaFile.addEventListener("change",()=>{ const file=mediaFile.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const id=activeThreadId();(state.messages[id]??=[]).push({id:`once-${Date.now()}`,from:"me",at:new Date().toISOString(),text:"View-once photo",imageUrl:String(reader.result),viewOnce:"one open",opened:false});save();render();toast("View-once photo added");mediaFile.value="";};reader.readAsDataURL(file);});
document.addEventListener("click",(event)=>{const mute=event.target.closest("[data-context-mute]");if(mute){const id=mute.dataset.contextMute;const list=state.notifications.mutedConversations;state.notifications.mutedConversations=list.includes(id)?list.filter(value=>value!==id):[...list,id];document.querySelector(".context-menu")?.remove();save();render();return;}const settings=event.target.closest("[data-context-notifications]");if(settings){document.querySelector(".context-menu")?.remove();state.route="settings";state.settingsTab="notifications";save();render();return;}const profile=event.target.closest("[data-context-profile]");if(profile){document.querySelector(".context-menu")?.remove();toast(`${getConversation(profile.dataset.contextProfile).name} profile opened`);return;}if(!event.target.closest(".context-menu"))document.querySelector(".context-menu")?.remove();});
document.addEventListener("click",(event)=>{ const target=event.target.closest("[data-action]"); if(!target||app.contains(target))return; const action=target.dataset.action;
  ...
  if(action==="share-canvas"){const canvas=document.querySelector("#whiteboard");const mode=canvas?.dataset.boardMode||"private";const id=activeThreadId();const image=canvas?.toDataURL("image/png")||"";if(mode==="shared")state.whiteboards[id]=image;(state.messages[id]??=[]).push({id:`canvas-${Date.now()}`,from:"me",at:new Date().toISOString(),text:mode==="shared"?"Updated shared canvas":"Sent a canvas",attachment:{name:mode==="shared"?"Shared canvas":"Canvas snapshot",size:"Live · encrypted",type:"CANVAS",state:mode==="shared"?"Everyone can edit":"Sent from private draft",boardId:id}});closeModal();save();render();toast(mode==="shared"?"Shared canvas updated":"Private canvas sent");return;}
  ...
  if(action==="create-group-confirm"){const name=document.querySelector("#group-name")?.value.trim()||"New group";const id=`group-${Date.now()}`;state.conversations.unshift({id,type:"dm",name,preview:"Group created",time:"now",unread:0,pinned:true,personIds:["nova","theo","maya"]});state.messages[id]=[{id:`${id}-1`,from:"me",at:new Date().toISOString(),text:"Group created"}];state.activeConversation=id;state.route="chats";state.chatHome=false;closeModal();save();render();toast(`${name} created`);return;}
  if(action==="close-once-photo"){const message=state.messages[activeThreadId()]?.find(item=>item.id===target.dataset.message);if(message)message.opened=true;closeModal();save();render();return;}
  if(target.dataset.applyRules){const message=state.messages[activeThreadId()]?.find(item=>item.id===target.dataset.applyRules);const timer=document.querySelector("#rule-timer")?.value;if(message&&timer&&timer!=="off")message.timer=timer;closeModal();save();render();toast("Encrypted message rule applied");}
  else handleAction(action,target);
});
