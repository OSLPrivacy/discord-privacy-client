...
fn should_relaunch_existing_session(id: NativeAppId, reason: NativeWindowHostReason) -> bool {
    matches!(
        id,
        NativeAppId::Telegram | NativeAppId::Signal | NativeAppId::Whatsapp | NativeAppId::Outlook
...
struct HostedWindow {
    ...
    borrowed_placement: Option<windows::BorrowedWindowPlacement>,
...
mod windows {
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::{
        ...
        MSG, PM_REMOVE, SWP_FRAMECHANGED, SWP_NOACTIVATE, SWP_NOZORDER, SWP_SHOWWINDOW,
        SW_MINIMIZE, SW_RESTORE, WDA_EXCLUDEFROMCAPTURE, WINDOWPLACEMENT, WS_CAPTION, WS_CHILD,
            ...
            ) == WarmHostAction::Reuse
            {
                let presented = unsafe {
                    if hosted.mode == DiscordSessionMode::Dedicated {
                        ...
                        present_borrowed_window(hosted.id, hosted.window as HWND, parent as HWND)
                            .is_ok()
                    }
                };
                if presented {
    ...
    pub(super) fn focus(state: &NativeWindowHostState) -> NativeWindowHostResult {
        ...
        let focused = unsafe {
            ...
            if hosted.mode == DiscordSessionMode::ExistingSession {
                ...
                // Foreground arbitration may legally reject these advisory
                // calls even though the borrowed window is restored and
                // visible. That is not a native-launch failure.
                let _ = BringWindowToTop(window);
                let _ = SetForegroundWindow(window);
                borrowed_focus_state_valid(IsWindowVisible(window) != 0, IsIconic(window) != 0)
                    && hosted
    ...
    unsafe fn adopt_existing_companion(
        ...
        if let Err(reason) = present_borrowed_window(id, window, owner) {
            restore_borrowed_window(window, previous_placement);
            return Err(reason);
        }
        let borrowed_control_shield = BorrowedControlShield::create(window, process_id)
        ...
        Ok(HostedWindow {
            ...
            previous_iconic,
            borrowed_placement: Some(previous_placement),
    ...
    unsafe fn restore_window(hosted: &HostedWindow) {
        ...
        if hosted_window_is_valid(hosted) {
            if hosted.mode == DiscordSessionMode::Dedicated {
                ...
                if let Some(saved) = hosted.borrowed_placement {
                    restore_borrowed_window(window, saved);
                }
            }
        }
    }
