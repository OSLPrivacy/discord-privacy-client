...
//! current composer value to prove it is empty or equals OSL's bounded local
//! cover. Prepared private text is reduced immediately to de-identified line
//! counts; only those counts can reach the carrier geometry planner.
...
impl NativeDiscordComposerState {
    ...
    pub fn verified_composer_bounds(&self) -> Option<AccessibilityBounds> {
        #[cfg(any(target_os = "windows", test))]
        {
            self.binding
                .lock()
                .ok()
                .and_then(|binding| binding.as_ref().map(|binding| binding.bounds))
        }
    ...
    /// Retain only a bounded visual summary after relay preparation succeeds.
    /// The exact draft must never be passed to this state object.
...
mod tests {
    ...
    fn prepared_visual_shape_is_scope_bound_and_consumed_once() {
        ...
        assert_eq!(plan.target.unwrap().line_count, 2);
        assert_eq!(plan.cover_lines.len(), 2);
