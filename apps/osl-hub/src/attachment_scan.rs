...
const ATTACHMENT_HANDLER_REGISTRY: &[(DetectedType, AttachmentHandler)] = &[
    ...
    (DetectedType::Pdf, AttachmentHandler::Pdf),
    (DetectedType::Docx, AttachmentHandler::Ooxml),
    (DetectedType::Xlsx, AttachmentHandler::Xlsx),
...
fn process_bytes(
    ...
    match handler_for(detected) {
        ...
        AttachmentHandler::Unsupported => output.uninspected_attachments.push(uninspected(
            attachment_id,
            path,
            detected.label(),
            UninspectedReason::Unsupported,
            "No registered byte-signature extractor supports this content".into(),
        )),
    }
}
...
fn extract_ooxml(
    ...
    for index in 0..zip.len() {
        ...
        if relevant_xml {
            if let Ok(text) = extract_xml_text(&member) {
...
mod tests {
    ...
    #[test]
    fn extracts_pdf_docx_xlsx_and_pptx_text_without_system_libraries() {
        let docx = zip_with(&[(
            "word/document.xml",
        ...
        let xlsx = zip_with(&[
            ...
            ("xl/workbook.xml", br#"<?xml version="1.0"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Sheet1" sheetId="1" r:id="rId1"/></sheets></workbook>"#),
            ...
            ("xl/worksheets/sheet1.xml", br#"<?xml version="1.0"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>secret: xlsx-value</t></is></c></row></sheetData></worksheet>"#),
        ]);
        for (bytes, expected_type, needle) in [
            (text_pdf("secret: pdf-value"), "pdf", "pdf-value"),
            (docx, "docx", "docx-value"),
            (xlsx, "xlsx", "xlsx-value"),
            (pptx, "pptx", "pptx-value"),
        ] {
            let result = scan_attachments(&[attachment(&bytes)], AttachmentAnalyzers::default());
            assert!(
                result
