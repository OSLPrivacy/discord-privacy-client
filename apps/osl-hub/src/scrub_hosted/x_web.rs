...
use super::verify_surface::{
    verify_on_independent_surface, HostedVerification, IndependentHostedSurface,
};
...
impl XArchive {
    ...
    pub fn ids(&self) -> impl Iterator<Item = &str> {
        self.tweet_ids.iter().map(String::as_str)
    }
}
...
impl<'a, D: XVisibleDelete, V: IndependentHostedSurface> XWebAdapter<'a, D, V> {
    pub fn new(archive: &'a XArchive, delete_ui: D, verifier: V) -> Self {
        Self {
            archive,
            delete_ui,
            verifier,
    ...
    pub fn delete_and_verify(
        &mut self,
        tweet_id: &str,
    ) -> Result<HostedVerification, XArchiveError> {
        if !self.archive.contains(tweet_id) {
            return Err(XArchiveError::TweetNotInArchive);
        }
...
mod tests {
    ...
    impl XVisibleDelete for Ui {
        fn delete_tweet_by_id(&mut self, id: &str) -> Result<(), XArchiveError> {
    ...
    fn scr_h10_ids_come_from_archive_and_are_reresolved_after_visible_delete() {
        ...
        assert_eq!(
            adapter.delete_and_verify("123456"),
            Ok(HostedVerification::VerifiedGone)
        );
        assert_eq!(adapter.delete_ui.0, ["123456"]);
        assert_eq!(
            adapter.delete_and_verify("999999"),
            Err(XArchiveError::TweetNotInArchive)
        );
