...
mod windows_viewer {
    ...
    unsafe extern "system" fn viewer_window_proc(
        ...
        let lookup = match viewers().lock() {
            Ok(mut guard) => Ok(if message == WM_NCDESTROY {
                guard
        ...
        // A poisoned registry means this window can no longer paint the pixels
        // it holds, so failing closed means getting it off the screen rather
        ...
        // A missing entry (already torn down) and a poisoned registry both fall
        // through to `DefWindowProcW`. Returning 0 for every message is what
        // stranded a visible, unclosable window.
        let previous = lookup.unwrap_or(None).unwrap_or(0);
        if previous == 0 {
