...
pub trait WhatsAppBackend: Send + Sync {
    ...
    /// Writes the carrier, then re-reads the exact composer/row digest.
...
pub struct WhatsAppSurfaceAdapter<B> {
    backend: B,
}

impl<B> WhatsAppSurfaceAdapter<B> {
    pub fn new(backend: B) -> Self { Self { backend } }
}

impl<B: WhatsAppBackend> WhatsAppSurfaceAdapter<B> {
    fn valid(&self, binding: &SurfaceBinding) -> bool {
        binding.app == AdapterAppId::Whatsapp && binding.generation != 0
    }
    ...
    fn placement_refused() -> PlacementReceipt {
        PlacementReceipt { status: PlacementStatus::NotPlaced, placed_sha256: None, elapsed_ms: 0 }
    }
    fn send_refused() -> SendReceipt {
        SendReceipt { outcome: SendOutcome::NotSent, elapsed_ms: 0 }
    }
}
...
impl<B: WhatsAppBackend> SurfaceAdapter for WhatsAppSurfaceAdapter<B> {
    ...
    fn locate(&self, target: &SurfaceTarget) -> Result<SurfaceBinding, AdapterRefusal> {
        ...
        if !self.valid(&binding) || binding.generation != target.generation {
    ...
    fn destination(&self, binding: &SurfaceBinding) -> Result<DestinationIdentity, AdapterRefusal> {
        if !self.valid(binding)
            || !matches!(binding.evidence, BindingEvidence::Accessibility { .. } | BindingEvidence::UserConfirmedVisualBinding { .. }) {
            return Err(AdapterRefusal::DestinationUnattested);
        }
        self.backend.destination(binding)
    }
    fn place(&self, binding: &SurfaceBinding, authorization: &PlacementAuthorization, carrier: &Carrier) -> PlacementReceipt {
        if !self.valid(binding)
        ...
        match self.read_state(binding) {
            Ok(state) if state.composer_is_empty && !state.composer_is_password_field && state.focused && !state.occluded && state.read_was_complete => {
                ...
                if receipt.status == PlacementStatus::Placed && receipt.placed_sha256.is_some() { receipt } else { Self::placement_refused() }
            }
            _ => Self::placement_refused(),
        }
    }
    fn commit(&self, binding: &SurfaceBinding, authorization: &SendAuthorization, placed: &PlacementReceipt) -> SendReceipt {
        if !self.valid(binding)
            || !is_send_evidence_admissible(&binding.evidence)
            || !same_scope(&binding.scope_binding_hash, authorization.scope_binding_hash())
            || !self.supports(adapter_profile::Capability::SendProtectedPayload)
            || placed.status != PlacementStatus::Placed || placed.placed_sha256.is_none() {
            return Self::send_refused();
        }
        match self.destination(binding) {
            Ok(destination) if destination.status == DestinationStatus::Attested
                && same_scope(&binding.scope_binding_hash, &destination.scope_binding_hash)
                && is_send_evidence_admissible(&destination.evidence) => self.backend.commit(binding, placed),
            _ => Self::send_refused(),
        }
    }
    fn paint_targets(&self, binding: &SurfaceBinding) -> Result<Vec<PaintTarget>, AdapterRefusal> {
        ...
        if targets.iter().any(|target| target.confidence == PaintConfidence::Exact
            && (target.carrier_sha256.len() != 64 || !target.carrier_sha256.bytes().all(|byte| byte.is_ascii_hexdigit()))) {
...
mod tests {
    ...
    fn binding() -> SurfaceBinding {
        SurfaceBinding::for_claimed_surface(AdapterAppId::Whatsapp, 1,
            BindingEvidence::Accessibility { tree: A11yTree::Both }, NodeRef::for_claimed_node(1),
            Some(NodeRef::for_claimed_node(2)), Bounds { x: 0, y: 0, width: 20, height: 20 }, 1, "scope")
    }
    ...
    #[test]
    fn t3_t11_c1_through_c10_requires_real_post_write_digest() {
        let adapter = WhatsAppSurfaceAdapter::new(Backend { writes: AtomicUsize::new(0), digest: true });
        let placed = adapter.place(&binding(), &PlacementAuthorization::for_scope("scope"), &Carrier("carrier".into()));
        assert_eq!(placed.status, PlacementStatus::Placed);
        assert_eq!(adapter.backend.writes.load(Ordering::SeqCst), 1);
        assert_eq!(adapter.commit(&binding(), &SendAuthorization::for_scope("scope"), &placed).outcome, SendOutcome::Sent);
    }
    #[test]
    fn t3_t13_c5_c8_refuses_unbound_paint_targets_and_pixel_send() {
        let adapter = WhatsAppSurfaceAdapter::new(Backend { writes: AtomicUsize::new(0), digest: true });
        let mut pixel = binding(); pixel.evidence = BindingEvidence::Pixel;
        ...
        impl WhatsAppBackend for BadPaint {
            fn capabilities(&self, _: u64) -> CapabilitySet { CapabilitySet::new() }
            fn locate(&self, _: &SurfaceTarget) -> Result<SurfaceBinding, AdapterRefusal> { Ok(binding()) }
            ...
            fn paint_targets(&self, _: &SurfaceBinding) -> Result<Vec<PaintTarget>, AdapterRefusal> {
                Ok(vec![PaintTarget { carrier_sha256: String::new(), rect: Bounds { x: 0, y: 0, width: 1, height: 1 }, clipped_by: None, confidence: PaintConfidence::Exact }])
            }
        }
        assert_eq!(WhatsAppSurfaceAdapter::new(BadPaint).paint_targets(&binding()), Err(AdapterRefusal::AccessibilityUnavailable));
    }
    #[test]
    fn c6_returning_ok_without_post_write_digest_is_refused() {
