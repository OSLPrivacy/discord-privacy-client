...
impl<B: WhatsAppBackend> SurfaceAdapter for WhatsAppSurfaceAdapter<B> {
    ...
    fn app(&self) -> AdapterAppId {
        AdapterAppId::Whatsapp
    }
    fn surface(&self) -> SurfaceKind {
        SurfaceKind::InstalledNativeClient
    }
    fn capabilities(&self, now: u64) -> CapabilitySet {
        self.backend.capabilities(now)
    }
    fn locate(&self, target: &SurfaceTarget) -> Result<SurfaceBinding, AdapterRefusal> {
    ...
    fn place(
        ...
        match self.read_state(binding) {
                    ...
                    && state.read_was_complete =>
            {
                ...
                if receipt.status == PlacementStatus::Placed && receipt.placed_sha256.is_some() {
                    receipt
                } else {
                    Self::refused()
                }
    ...
    fn paint_targets(&self, binding: &SurfaceBinding) -> Result<Vec<PaintTarget>, AdapterRefusal> {
        ...
        if targets.iter().any(|target| {
            target.carrier_sha256.len() != 64
                || !target.carrier_sha256.bytes().all(|b| b.is_ascii_hexdigit())
        }) {
...
mod tests {
    ...
    impl WhatsAppBackend for Backend {
        ...
        fn read_state(&self, _: &SurfaceBinding) -> Result<SurfaceState, AdapterRefusal> {
            Ok(SurfaceState {
                composer_text_sha256: "before".into(),
        ...
        fn place(&self, _: &SurfaceBinding, _: &Carrier) -> PlacementReceipt {
            ...
            PlacementReceipt {
                ...
                placed_sha256: self.digest.then(|| "a".repeat(64)),
                elapsed_ms: 1,
            }
        }
    ...
    fn t3_t11_c1_through_c10_requires_a_real_post_write_digest() {
        let adapter = WhatsAppSurfaceAdapter::new(Backend {
        ...
        assert_eq!(
            adapter
                .commit(&b, &SendAuthorization::for_scope("scope"), &placed)
                .outcome,
            SendOutcome::Sent
        );
        assert_eq!(
    ...
    fn t3_t13_c5_c8_refuses_an_unbound_paint_target_and_pixel_send() {
        ...
        assert_eq!(
            adapter
                .commit(&pixel, &SendAuthorization::for_scope("scope"), &placed)
                .outcome,
            SendOutcome::NotSent
        );
        ...
        assert_eq!(
            WhatsAppSurfaceAdapter::new(BadPaint).paint_targets(&binding()),
