...
pub const REGISTRATION_WAIT_LIMIT: Duration = Duration::from_secs(30);
...
pub struct RegistrationBarrierOutcome {
    ...
    pub keyserver_available: bool,
    pub identity_unchanged: bool,
    pub ready: bool,
}

fn registration_wait_decision(
    state: ipc::state::CloudRegistrationState,
    keyserver_available: bool,
    identity_unchanged: bool,
) -> RegistrationWaitDecision {
    if !identity_unchanged {
        return RegistrationWaitDecision::Fail;
    }
    match state {
        ipc::state::CloudRegistrationState::Registered if keyserver_available => {
            RegistrationWaitDecision::Ready
        }
        ipc::state::CloudRegistrationState::Conflict
        | ipc::state::CloudRegistrationState::Offline => RegistrationWaitDecision::Fail,
        _ => RegistrationWaitDecision::Wait,
    }
}

/// One non-blocking readiness read for latency-sensitive manual QA gestures.
/// Background startup remains responsible for registration; a user's Enter
/// either proceeds immediately against the already-registered identity or
/// fails closed without holding their draft for the 30-second barrier.
pub fn registered_transport_now(core: &HubCoreState) -> RegistrationBarrierOutcome {
    let identity_available = core
        .osl
        .identity
        .lock()
        .ok()
        ...
        .is_some();
    let keyserver_available = core.osl.has_keyserver();
    let state = core.osl.cloud_registration_state();
    let ready = identity_available
        && keyserver_available
        && state == ipc::state::CloudRegistrationState::Registered;
    RegistrationBarrierOutcome {
        terminal_state: if ready {
            "registered"
        } else if !identity_available {
            "identity_unavailable"
        } else {
            match state {
                ipc::state::CloudRegistrationState::Conflict => "conflict",
                ipc::state::CloudRegistrationState::Offline => "offline",
                _ => "unavailable",
            }
        },
        keyserver_available,
        identity_unchanged: identity_available,
        ready,
    }
}

/// Wait only for the disposable QA identity's background registration.
/// Production message paths never call this helper.
pub fn wait_for_registered_transport(core: &HubCoreState) -> RegistrationBarrierOutcome {
        ...
        .and_then(|identity| identity.as_ref().map(|value| value.user_id.clone()))
    {
        ...
        None => {
            return RegistrationBarrierOutcome {
                terminal_state: "identity_unavailable",
                keyserver_available: core.osl.has_keyserver(),
    ...
    loop {
        ...
        let keyserver_available = core.osl.has_keyserver();
        let state = core.osl.cloud_registration_state();
        match registration_wait_decision(state, keyserver_available, identity_unchanged) {
            RegistrationWaitDecision::Ready => {
                return RegistrationBarrierOutcome {
            ...
            RegistrationWaitDecision::Fail => {
                let terminal_state = if !identity_unchanged {
                    "identity_changed"
                } else {
                    match state {
                        ipc::state::CloudRegistrationState::Conflict => "conflict",
            ...
            RegistrationWaitDecision::Wait if Instant::now() >= deadline => {
                return RegistrationBarrierOutcome {
            ...
            RegistrationWaitDecision::Wait => std::thread::sleep(Duration::from_millis(100)),
        }
    }
}
...
/// Re-open the fixed device-sealed QA key and require it to match the key
/// already installed in the process. This is deliberately narrower than
/// accepting any populated file-storage key.
pub fn require_installed_device_bound_storage_key() -> Result<[u8; DEVICE_KEY_BYTES], String> {
    let account_dir = pairing_root_dir()?;
    let bytes = crate::atomic_file::read_recoverable_bounded(
        &account_dir.join(DEVICE_KEY_FILENAME),
        MAX_DEVICE_KEY_RECORD_BYTES,
        "Discord QA device key",
    )?
    .ok_or_else(|| "Discord QA device key is unavailable".to_owned())?;
    let sealer = crate::password_lifecycle::persistent_sealer()?;
    let sealed_key = decode_record(&bytes, sealer.as_ref())?;
    let installed = ipc::main_password::get_file_storage_key()
        .ok_or_else(|| "Discord QA device key is not installed".to_owned())?;
    let differs = sealed_key
        .iter()
        .zip(installed.iter())
        .fold(0u8, |difference, (left, right)| difference | (left ^ right));
    if differs != 0 {
        return Err("Discord QA installed device key does not match its sealed record".to_owned());
    }
    Ok(installed)
}
...
pub fn ensure_disposable_identity(core: &HubCoreState) -> Result<(), String> {
    ...
    let result = crate::password_lifecycle::create_native_identity(
        core,
        Some(crate::password_lifecycle::IdentityCreationOwnerAuthorization::ExplicitOwnerSignoff),
    )?;
    drop(result);
    Ok(())
}
...
/// QA controller offers live at the fixed OSL core root, not inside the
/// currently active identity slot. Identity selection deliberately changes
/// `osl_config_dir()`, so all pairing callers must resolve the base explicitly.
pub fn pairing_root_dir() -> Result<std::path::PathBuf, String> {
    keystore::osl_base_dir().map_err(|_| "Discord QA pairing storage is unavailable".to_owned())
}

/// Resolve only the peer proven by the controller-installed offer and the
/// locally generated pairing receipt. Stale People rows cannot influence QA
/// routing, and any offer/status/hash/key-stability mismatch fails closed.
pub fn verified_pairing_person_id(
    account_dir: &Path,
    core: &HubCoreState,
    ...
    .ok_or_else(|| "Discord QA peer offer is unavailable".to_owned())?;
    let peer_offer = decode_public_offer(&peer_bytes)?;
    let status_bytes = crate::atomic_file::read_recoverable_bounded(
        &account_dir.join(PAIRING_STATUS_FILENAME),
        MAX_PUBLIC_OFFER_BYTES,
        "Discord QA pairing status",
    )?
    .ok_or_else(|| "Discord QA pairing status is unavailable".to_owned())?;
    let status: PairingStatus = serde_json::from_slice(&status_bytes)
        .map_err(|_| "Discord QA pairing status is invalid".to_owned())?;
    if status.version != PUBLIC_OFFER_VERSION
        || !status.verified
    ...
    let matching: Vec<_> = people
        .iter()
        .filter(|person| person.person_id == status.peer_person_id)
        .collect();
    if matching.len() != 1 {
        return Err("Discord QA pairing does not resolve one local friend".to_owned());
    }
    let person = matching[0];
    if !person.safety_number_verified
        || person.pending_key_change
        || person.osl_user_id != status.peer_osl_user_id
        || person.safety_number != status.peer_safety_number
    {
        return Err("Discord QA pairing is not verified to stable current keys".to_owned());
    }
    Ok(person.person_id.clone())
}
...
mod tests {
    ...
    // Shared crate-wide, not module-local: services.rs, cleanup.rs, and
    ...
    fn pairing_root_ignores_the_active_identity_slot() {
        ...
        keystore::set_active_account_dir(Some(active.clone()));
        assert_eq!(keystore::osl_config_dir().unwrap(), active);
        assert_eq!(pairing_root_dir().unwrap(), root);
    }
    ...
    #[test]
    fn qa_transport_requires_registered_identity_and_client() {
        use ipc::state::CloudRegistrationState as State;

        assert_eq!(
            registration_wait_decision(State::Registered, true, true),
            RegistrationWaitDecision::Ready
        );
        assert_eq!(
            registration_wait_decision(State::Registered, false, true),
            RegistrationWaitDecision::Wait
        );
        assert_eq!(
            registration_wait_decision(State::Pending, true, true),
            RegistrationWaitDecision::Wait
        );
        assert_eq!(
            registration_wait_decision(State::Offline, true, true),
            RegistrationWaitDecision::Fail
        );
        assert_eq!(
            registration_wait_decision(State::Registered, true, false),
            RegistrationWaitDecision::Fail
        );
        assert!(REGISTRATION_WAIT_LIMIT <= Duration::from_secs(30));
    }
