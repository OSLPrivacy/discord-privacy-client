...
pub const TEST_ONLY_RUNTIME_SWITCH_LIST_NAME: &str = TEST_ONLY_RUNTIME_SWITCH_LIST;
...
impl PasswordScreenAccess {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::RequirePasswordScreen => PASSWORD_SCREEN_ACCESS_REQUIRED,
            Self::SkipPasswordScreenForTest => PASSWORD_SCREEN_ACCESS_SKIP_FOR_TEST,
        }
    }

    fn from_str(value: &str) -> Option<Self> {
        match value {
            PASSWORD_SCREEN_ACCESS_REQUIRED => Some(Self::RequirePasswordScreen),
            PASSWORD_SCREEN_ACCESS_SKIP_FOR_TEST => Some(Self::SkipPasswordScreenForTest),
            _ => None,
        }
    }
}
...
impl SafeSending {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::LiveSendRequiresAuthority => SAFE_SENDING_LIVE_AUTHORITY_REQUIRED,
            Self::DryRunSendForTest => SAFE_SENDING_DRY_RUN_FOR_TEST,
        }
    }

    fn from_str(value: &str) -> Option<Self> {
        match value {
            SAFE_SENDING_LIVE_AUTHORITY_REQUIRED => Some(Self::LiveSendRequiresAuthority),
            SAFE_SENDING_DRY_RUN_FOR_TEST => Some(Self::DryRunSendForTest),
            _ => None,
        }
    }
}
...
pub const TEST_ONLY_RUNTIME_SWITCHES: &[RunTimeSwitchDescriptor] = &[
    RunTimeSwitchDescriptor {
        ...
        default: PASSWORD_SCREEN_ACCESS_REQUIRED,
        allowed: &[
    ...
    RunTimeSwitchDescriptor {
        ...
        default: SAFE_SENDING_LIVE_AUTHORITY_REQUIRED,
        allowed: &[
...
/// The declared switch list in the older spec-table shape. `RunTimeSwitchSpec`
/// and `RunTimeSwitchList` remain so callers that enumerate the published
/// switch contract (name, default, allowed values) keep one authority to read.
...
const TEST_ONLY_RUNTIME_SWITCH_SPECS: &[RunTimeSwitchSpec] = &[
    RunTimeSwitchSpec {
        name: PASSWORD_SCREEN_ACCESS_SWITCH,
...
const TEST_ONLY_RUNTIME_SWITCH_LIST_TABLE: RunTimeSwitchList = RunTimeSwitchList {
    name: TEST_ONLY_RUNTIME_SWITCH_LIST_NAME,
    switches: TEST_ONLY_RUNTIME_SWITCH_SPECS,
};

pub fn test_only_runtime_switch_list() -> &'static RunTimeSwitchList {
    &TEST_ONLY_RUNTIME_SWITCH_LIST_TABLE
}
...
/// The startup choice report in the shape the wiring checks print.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub fn test_only_runtime_switch_choice_report(
    ...
    TestOnlyRunTimeSwitchChoiceReport {
        password_screen_access: switches.password_screen_access.as_str(),
        safe_sending: switches.safe_sending.as_str(),
...
pub fn resolve_test_only_runtime_switches(
    ...
    Ok(ResolvedTestOnlyRunTimeSwitches {
        password_screen_access: PasswordScreenAccess::from_str(password_screen_access)
            .expect("validated password_screen_access value must map to a variant"),
        safe_sending: SafeSending::from_str(safe_sending)
            .expect("validated safe_sending value must map to a variant"),
    })
}
...
fn switch_spec(switch: &str) -> Option<&'static RunTimeSwitchSpec> {
    TEST_ONLY_RUNTIME_SWITCH_SPECS
        .iter()
...
pub fn assert_runtime_switch_status_safe(lines: &[RuntimeSwitchStatusLine]) -> Result<(), String> {
    ...
    for line in lines {
        ...
        if line.name == SAFE_SENDING_SWITCH
