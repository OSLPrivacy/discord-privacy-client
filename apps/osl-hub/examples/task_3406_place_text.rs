...
mod windows_place_text {
    ...
    pub fn run() -> Result<(), CommandError> {
        ...
        let private_chars = count_private_canary_chars_reaching_clipboard(
            &private_canary,
        ...
        if private_chars != 0 {
            return Err(CommandError::exit1(format!(
                "{private_chars} private canary characters reached the clipboard"
            )));
        }
        Ok(())
    }
    ...
    fn count_private_canary_chars_reaching_clipboard(canary: &str, observations: &[&str]) -> usize {
        canary
            .chars()
