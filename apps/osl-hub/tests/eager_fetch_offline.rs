...
impl CipherStoreTransport for Network {
    fn fetch(&mut self, blob_id: &str, _: &[u8]) -> Result<Vec<u8>, String> {
        if !self.online { return Err("network is down".to_owned()); }
        self.blobs.get(blob_id).cloned().ok_or_else(|| "blob gone".to_owned())
    }
...
#[derive(Default)]
struct Local { plaintext: BTreeMap<String, Vec<u8>>, opens: usize }

impl Local {
    fn open(&mut self, blob_id: &str) -> Result<Vec<u8>, String> {
        self.opens += 1;
        self.plaintext.get(blob_id).cloned().ok_or_else(|| "not persisted".to_owned())
    }
}

impl LocalMessageStore for Local {
    fn decrypt_and_persist(&mut self, blob_id: &str, ciphertext: &[u8]) -> Result<(), String> {
        self.plaintext.insert(blob_id.to_owned(), ciphertext.to_vec());
        Ok(())
    }
...
fn pointer(id: &str) -> PointerArrival {
    PointerArrival { blob_id: id.to_owned(), fetch_cap: vec![7; 32], manage_cap: vec![9; 32] }
}
...
fn t6_t28_pointer_fetches_before_open_and_offline_burn_survives_restart_without_eviction() {
    ...
    let mut network = Network { online: true, blobs: BTreeMap::from([("a".to_owned(), b"plain".to_vec())]), burned: vec![] };
    ...
    assert_eq!(local.open("a").unwrap(), b"plain", "open reads the eager-persisted local copy while hard-down");
    assert_eq!(local.opens, 1);

    let mut driver = EagerFetchDriver::new(network, local, queue);
    driver.burn_offline(&pointer("a")).unwrap();
    let (mut network, mut local, queue) = driver.into_parts();
    assert!(local.open("a").is_err(), "local destruction does not wait for reconnect");
    assert_eq!(queue.pending().unwrap().len(), 1);
    assert!(!std::fs::read(&queue_path).unwrap().windows(b"plain".len()).any(|part| part == b"plain"));
    ...
    assert_eq!(network.burned, vec![("a".to_owned(), vec![9; 32])], "manage capability is sent unchanged");
    ...
    assert_eq!(queue.pending().unwrap().len(), MAX_PENDING_BURNS, "a full queue never evicts a live record");
}
