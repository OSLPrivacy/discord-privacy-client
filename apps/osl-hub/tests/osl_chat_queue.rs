...
impl RawBackend for Memory {
    fn write_blob(&self, key: &str, blob: &[u8]) -> Result<(), SecureLocalStoreError> {
        self.0.lock().unwrap().insert(key.to_owned(), blob.to_vec()); Ok(())
    }
...
fn t14_t12_full_queue_refuses_without_evicting_and_reconnect_reuses_the_envelope() {
    ...
    queue.enqueue_encrypted("chat-message-1", vec![9, 8, 7]).unwrap();

    assert!(matches!(
        queue.enqueue_encrypted("chat-message-2", vec![6]),
        Err(OfflineSendQueueError::Full { capacity: 1 })
    ));
    assert_eq!(queue.pending().unwrap().len(), 1, "Q1 must retain the live record");

    let sent = Arc::new(Mutex::new(Vec::new()));
    let sent_by_callback = sent.clone();
    queue.drain_on_reconnect(move |pending| -> Result<(), &'static str> {
        sent_by_callback.lock().unwrap().push((pending.idempotency_key.clone(), pending.encrypted_envelope.clone()));
        Ok(())
    }).unwrap();
    assert_eq!(*sent.lock().unwrap(), vec![("chat-message-1".to_owned(), vec![9, 8, 7])]);
