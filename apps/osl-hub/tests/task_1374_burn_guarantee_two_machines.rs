...
fn run() -> Result<(), String> {
    ...
    assert!(first_read.exact_mark);
    assert_eq!(first_read.count, 1);
    assert!(second_read.exact_mark);
    assert_eq!(second_read.count, 1);
    assert_eq!(first_burn_deleted, 1);
    assert_eq!(second_burn_deleted, 1);
    assert_eq!(first_reopened.count, 0);
...
fn burn_both_sides_once(first: &Machine, second: &Machine) -> Result<(usize, usize), String> {
    let first_deleted = {
        let (core, broker, token) = first.open_app(30)?;
        broker::burn_local_protected_context(&core, &broker, &token)?
    };
    let second_deleted = {
        let (core, broker, token) = second.open_app(40)?;
        broker::burn_local_protected_context(&core, &broker, &token)?
    };
