...
use osl_privacy_hub::{
    autoscrub_run::{
        ...
        OwnSessionProofInput, OwnedAccountRegistry, ReviewedBatchInput, RunConsentBatchInput,
...
impl OwnedAccountRegistry for Owned {
    fn confirms_owned(&self, _: &str, _: ServiceKind, _: &str) -> bool {
        true
    }
}
...
fn inputs(
    ...
    let now = now_ms();
    let batch = ReviewedBatchInput {
        plan_fingerprint: "fixture-plan".into(),
        findings_fingerprint: "fixture-findings".into(),
        item_keys: vec![item_key()],
    };
    (
        RunManifestInput {
            run_id: run_id.into(),
            provider_id: "imap".into(),
        ...
        OwnSessionProofInput {
            provider_id: "imap".into(),
            account_id: ACCOUNT.into(),
            session_kind: "owner_provisioned_credential".into(),
            session_epoch: "fixture-epoch".into(),
        ...
        DeleteCapabilityInput {
            mechanism: "documented_provider_delete_api".into(),
            stop_on: vec![
                "owner_stop".into(),
                "manifest_drift".into(),
                "deadline".into(),
...
fn scr_v4_seed_7e57_replays_stop_revoke_and_stale_events_without_authority_escape() {
    ...
    ledger
        .step(OWNER, NativeEntitlement::Confirmed, &request)
        .unwrap();
    let duplicate = ledger.step(OWNER, NativeEntitlement::Confirmed, &request);
    if let Ok(authorization) = duplicate {
        assert_eq!(
            authorization.items_authorized, 1,
            "a duplicate event cannot create a second destructive authorization"
        );
    }
    ...
    assert!(matches!(
        ledger.authorize_imap_prepare(OWNER, NativeEntitlement::Confirmed, ACCOUNT, "INBOX", ITEM),
        Err(AutoScrubRunError::RunHalted) | Err(AutoScrubRunError::AuthorityRevoked)
    ));

    // Reordered/stale event cannot resurrect a halted run or mint a receipt.
    assert!(matches!(
        ledger.step(OWNER, NativeEntitlement::Confirmed, &request),
