...
use osl_privacy_hub::service_connections::{
    read_gmx_shared_mailbox_messages_paged, SharedMailLabel, SharedMailMessageRecord,
    SharedMailboxPagedRead, SharedMailboxPagingConfig, SharedMailboxPagingPause,
    SharedMailboxPagingStop, SharedMailboxSnapshot,
};

const SIGNED_IN: &str = "scrub-owner@gmx.test";
...
fn seeded_gmx_test_mailbox() -> SharedMailboxSnapshot {
    let sent = (0..120).map(|index| {
        SharedMailMessageRecord::new(
            "Sent",
            format!("sent-gmx-{index:03}"),
            format!("GMX scrub message {index:03}"),
            1_786_360_000 + i64::from(index) * 60,
            SIGNED_IN,
            format!("Seeded GMX scrub body {index:03}."),
        )
    });
    let inbox = [
        SharedMailMessageRecord::new(
...
#[test]
fn task_3066_gmx_pages_with_pause_and_stops_during_page_two() {
    let mailbox = seeded_gmx_test_mailbox();
    ...
    let full = read_gmx_shared_mailbox_messages_paged(
        &mailbox,
        "Sent",
        config,
        &mut full_pause,
    ...
    .expect("GMX full folder read pages");
    ...
    let stopped = read_gmx_shared_mailbox_messages_paged(
        &mailbox,
        "Sent",
        config,
        &mut stop_pause,
    ...
    .expect("GMX stopped folder read pages");
    ...
    println!("TASK3066 direct_reader=gmx_shared_mailbox_paged_reader");
    println!("TASK3066 folder=Sent folder_message_count={sent_folder_message_count}");
    println!("TASK3066 page_size={PAGE_SIZE}");
    println!("TASK3066 configured_pause_ms={PAUSE_MS}");
    println!("TASK3066 full_read.page_count={}", full.pages.len());
    println!("TASK3066 full_read.messages_read={}", full.messages_read);
    println!("TASK3066 full_read.page_lengths={:?}", page_lengths(&full));
    println!("TASK3066 full_read.stopped={}", full.stopped);
    println!("TASK3066 full_read.pause_count={}", full_pause.calls.len());
    for (page, ms) in &full_pause.calls {
        println!("TASK3066 full_read.pause_after_page={page} pause_ms={ms}");
    }
    println!(
        "TASK3066 stop_run.stop_requested_during_page={}",
        stop.requested_on_page.unwrap_or_default()
    );
    println!("TASK3066 stop_run.stopped={}", stopped.stopped);
    println!(
        "TASK3066 stop_run.stopped_on_page={}",
        stopped.stopped_on_page.unwrap_or_default()
    );
    println!("TASK3066 stop_run.messages_read={}", stopped.messages_read);
    println!("TASK3066 stop_run.messages_read_between_40_and_80={stopped_between_40_and_80}");
    println!("TASK3066 stop_run.page_count={}", stopped.pages.len());
    println!(
        "TASK3066 stop_run.page_lengths={:?}",
        page_lengths(&stopped)
    );
    println!("TASK3066 stop_run.page_2_messages_read={page_two_read}");
    println!("TASK3066 stop_run.pause_count={}", stop_pause.calls.len());
    for (page, ms) in &stop_pause.calls {
        println!("TASK3066 stop_run.pause_after_page={page} pause_ms={ms}");
    }
