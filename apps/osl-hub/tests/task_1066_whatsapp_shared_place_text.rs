#![cfg_attr(task1066_direct, allow(dead_code))]
...
use crate::native_whatsapp_adapter::{
    drive_whatsapp_composer_placement, WhatsAppPlacementStatus, WHATSAPP_CARRIER_PREFIX,
    WHATSAPP_DESKTOP_PROCESS_NAME, WHATSAPP_ROOT_WINDOW_CLASS, WHATSAPP_UIA2_WINDOW_PLAN,
};
#[cfg(not(task1066_direct))]
...
mod fixture {
    include!("fixtures/task_1066_marked_bytes.rs");
}

const EXPECTED_ORIGINAL_FIXTURE_BYTES: &[u8] = b"OSL1.WA.WA1066-MARKED-BYTES-7C41";
const SHELL_HWND: isize = 10_660;
const WEBVIEW_HWND: isize = 10_661;
const RENDERER_HWND: isize = 10_662;
const SHELL_PROCESS_ID: u32 = 10_660;
const WEBVIEW_PROCESS_ID: u32 = 10_661;
...
#[test]
fn confirmed_whatsapp_box_uses_the_shared_place_text_job_with_exact_readback() {
    assert_eq!(
        fixture::MARKED_BYTES,
        EXPECTED_ORIGINAL_FIXTURE_BYTES,
        "task 1066 fixture changed by one byte"
    );
    ...
    let receipt = drive_whatsapp_composer_placement(&host, marked, false);
    ...
    let visible_value = host
        .value
        .borrow()
        .clone()
        .expect("shared job leaves the marked string visibly in the confirmed box");
    assert_eq!(visible_value.as_bytes(), fixture::MARKED_BYTES);
    let writes = host.writes.borrow();
    assert_eq!(writes.len(), 1, "the shared placement job writes once");
    assert_eq!(writes[0], fixture::MARKED_BYTES);

    println!("TASK1066_CONFIRMED_WHATSAPP_BOX_COUNT=1");
    println!("TASK1066_MARKED_STRING_VISIBLE={visible_value}");
    println!("TASK1066_FIXTURE_BYTES={}", fixture::MARKED_BYTES.len());
    println!("TASK1066_READBACK_BYTES={}", receipt.readback_bytes);
    println!("TASK1066_READBACK_EXACT={}", receipt.readback_exact);
    println!("TASK1066_SHARED_PLACE_WRITES={}", writes.len());
    ...
    assert!(adapter_source.contains("placed.readback_exact = receipt.readback_exact;"));
    assert!(adapter_source.contains("placed.readback_bytes = receipt.readback_bytes;"));
}
