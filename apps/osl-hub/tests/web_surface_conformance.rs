...
fn profile() -> adapter_profile::ProfilePayload {
    adapter_profile::ProfilePayload {
        domain: "osl/adapter-profile/v1".into(), schema_version: 1,
        adapter_id: "fixture.web".into(),
        app: adapter_profile::AppDescriptor { stable_id: "x".into(), display_name: "X".into(), service_family: "messaging".into(), min_app_version: None },
        revision: adapter_profile::ProfileRevision { number: 1, label: "fixture".into() },
        issued_at_unix_seconds: 1, expires_at_unix_seconds: u64::MAX,
        support: adapter_profile::SupportLevel::Supported,
        authority: adapter_profile::AuthorityRequirements { user_consent_required: true, account_binding_required: true, release_authority_required: true, harmless_canary_required: true },
        selectors: vec![], fallbacks: vec![],
        canary: adapter_profile::HarmlessCanary { selector: adapter_profile::SelectorKind::AppRoot, expected_text: "Messages".into(), max_age_seconds: 1 },
    }
}

fn binding(generation: u64, pixel: bool) -> SurfaceBinding {
    SurfaceBinding::for_claimed_surface(AdapterAppId::X, generation,
        if pixel { BindingEvidence::Pixel } else { BindingEvidence::Accessibility { tree: A11yTree::WebAx } },
        NodeRef::for_claimed_node(1), Some(NodeRef::for_claimed_node(2)),
        Bounds { x: 0, y: 0, width: 10, height: 10 }, 1, "opaque-scope")
}

impl WebSurfaceBackend for Fixture {
    fn capabilities(&self, _: &adapter_profile::ProfilePayload, _: u64) -> CapabilitySet {
        [adapter_profile::Capability::PlaceProtectedPayload, adapter_profile::Capability::SendProtectedPayload].into_iter().collect()
    }
    ...
    fn wake_accessibility(&self) -> Result<(), AdapterRefusal> {
        ...
        if self.wake_fails { Err(AdapterRefusal::AccessibilityUnavailable) } else { Ok(()) }
    }
...
fn adapter(fixture: Fixture) -> WebSurfaceAdapter<Fixture> { WebSurfaceAdapter::new(AdapterAppId::X, profile(), fixture) }
fn current_target() -> SurfaceTarget { SurfaceTarget { app: AdapterAppId::X, surface: SurfaceKind::FixedOfficialWebOrigin, generation: 7 } }
...
fn web_w3_c1_to_c10_conformance() {
    ...
    let b = a.locate(&current_target()).unwrap();
    assert_eq!(a.place(&b, &PlacementAuthorization::for_scope("opaque-scope"), &Carrier("carrier".into())).status, PlacementStatus::NotPlaced);

    // C3/C4: neither Unknown nor Changed may commit.
    for mode in [DestinationMode::Unknown, DestinationMode::Changed] {
        let a = adapter(Fixture { writes: AtomicUsize::new(0), wakeups: AtomicUsize::new(0), password: false, pixel: false, destination: mode, unknown_send: false, empty_exact_target: false, wake_fails: false });
        let b = a.locate(&current_target()).unwrap();
        assert_eq!(a.commit(&b, &SendAuthorization::for_scope("opaque-scope"), &placed()).outcome, SendOutcome::NotSent);
    }

    // C5/C8: Pixel cannot commit or yield an Exact paint target; exact needs a carrier digest.
    let a = adapter(Fixture { writes: AtomicUsize::new(0), wakeups: AtomicUsize::new(0), password: false, pixel: true, destination: DestinationMode::Attested, unknown_send: false, empty_exact_target: false, wake_fails: false });
    let b = a.locate(&current_target()).unwrap();
    assert_eq!(a.commit(&b, &SendAuthorization::for_scope("opaque-scope"), &placed()).outcome, SendOutcome::NotSent);
    assert_eq!(a.paint_targets(&b), Err(AdapterRefusal::AccessibilityUnavailable));
    let a = adapter(Fixture { writes: AtomicUsize::new(0), wakeups: AtomicUsize::new(0), password: false, pixel: false, destination: DestinationMode::Attested, unknown_send: false, empty_exact_target: true, wake_fails: false });
    let b = a.locate(&current_target()).unwrap();
    assert_eq!(a.paint_targets(&b), Err(AdapterRefusal::AccessibilityUnavailable));
