...
use osl_privacy_hub::{
    eager_fetch::{CipherStoreTransport, EagerFetchDriver, EncryptedBurnQueue, LocalMessageStore, PointerArrival},
...
impl CipherStoreTransport for Transport {
    fn fetch(&mut self, blob_id: &str, _: &[u8]) -> Result<Vec<u8>, String> {
        self.0.get(blob_id).cloned().ok_or_else(|| "missing payload".to_owned())
    }
    fn burn(&mut self, _: &str, _: &[u8]) -> Result<(), String> { Ok(()) }
}
...
impl LocalMessageStore for Store {
    ...
    fn destroy_local(&mut self, _: &str) -> Result<(), String> { Ok(()) }
}
...
fn t14_t11_payload_is_local_before_the_user_opens_the_conversation() {
    ...
    transport.0.insert("pointer-1".to_owned(), b"ciphertext".to_vec());
    let queue = EncryptedBurnQueue::new(&path, [7; 32]);
    let mut driver = EagerFetchDriver::new(transport, Store::default(), queue);
    let pointer = PointerArrival { blob_id: "pointer-1".to_owned(), fetch_cap: vec![1], manage_cap: vec![2] };
