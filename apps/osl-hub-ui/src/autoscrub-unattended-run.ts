...
export async function autoscrubUnattendedProductionRun(rawContract: unknown, nativeInvoke: typeof invoke = invoke): Promise<AutoscrubUnattendedRunResult> {
  ...
  const started = parseAutoscrubUnattendedRunStarted(await nativeInvoke(AUTOSCRUB_UNATTENDED_RUN_COMMAND, { contract }).catch(() => null));
