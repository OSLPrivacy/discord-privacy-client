...
export function passwordSecuritySettingsContent(
  ...
  const passwordAction = state.bootstrapStatus === "setupRequired"
    ...
    : state.bootstrapStatus === "passwordRequired"
      ...
      : state.bootstrapStatus === "identityKeyLost"
        ? `<span class="setting-status machine-fact"><span class="dot"></span>This device can no longer open this account</span><button class="button primary" data-onboarding-action="import">Restore with recovery phrase</button>`
        : `<span class="setting-status machine-fact"><span class="dot"></span>Password configured and unlocked</span><button class="button" type="button" data-lock-session="now">Lock now</button>`;
  ...
  const roles = state.passwordRoleStatus
    ...
    : `<div class="settings-unavailable"><strong class="machine-fact">Password roles unavailable</strong><span>Unlock OSL and reopen Settings.</span></div>`;
  return `<section class="settings-section password-security"><header><div><h3 class="machine-fact">Password & security</h3><p>Protects encrypted storage on this device.</p></div><div class="settings-actions">${passwordAction}</div></header><details class="settings-disclosure"><summary class="machine-fact">Alternate passwords</summary><div>${roles}</div></details></section>`;
}
