...
describe("native window host IPC", () => {
  ...
  it("hosts one exact allowlisted app through the narrow command", async () => {
    ...
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "discord", discordSessionMode: "existingSession", discordTakeover: "borrowExisting" });
  });
  ...
  it("defaults malformed or missing Discord session choices to the dedicated profile", async () => {
    ...
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "discord", discordSessionMode: "dedicated", discordTakeover: "borrowExisting" });
  });
  ...
  it("allows explicit existing sessions only for the supported native apps", async () => {
    ...
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "telegram", discordSessionMode: "existingSession", discordTakeover: "borrowExisting" });
    ...
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "whatsapp", discordSessionMode: "existingSession", discordTakeover: "borrowExisting" });
    ...
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "signal", discordSessionMode: "existingSession", discordTakeover: "borrowExisting" });
  });
  ...
  it("hosts Outlook only as an existing native account", async () => {
    ...
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "outlook", discordSessionMode: "existingSession", discordTakeover: "borrowExisting" });
  });

  it("forwards a consented Discord takeover and refuses to take over anything else", async () => {
    const response = { id: "discord", status: "hosted", reason: "none", mode: "existingNativeCompanion", captureProtected: false };
    mocks.invoke.mockResolvedValueOnce(response);
    await expect(hostNativeAppWindow("discord", "existingSession", "quitAndRelaunch")).resolves.toEqual(response);
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "discord", discordSessionMode: "existingSession", discordTakeover: "quitAndRelaunch" });

    mocks.invoke.mockResolvedValueOnce({ id: "discord", status: "hosted", reason: "none", mode: "ownedBorderless", captureProtected: false });
    await expect(hostNativeAppWindow("discord", "dedicated", "quitAndRelaunch")).resolves.toMatchObject({ id: "discord", status: "hosted" });
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "discord", discordSessionMode: "dedicated", discordTakeover: "borrowExisting" });

    mocks.invoke.mockResolvedValueOnce({ id: "telegram", status: "hosted", reason: "none", mode: "existingNativeCompanion", captureProtected: false });
    await expect(hostNativeAppWindow("telegram", "existingSession", "quitAndRelaunch")).resolves.toMatchObject({ id: "telegram", status: "hosted" });
    expect(mocks.invoke).toHaveBeenCalledWith("host_native_app_window", { appId: "telegram", discordSessionMode: "existingSession", discordTakeover: "borrowExisting" });
  });

  it("defaults every malformed or missing takeover choice to borrowing", () => {
    expect(parseNativeDiscordTakeover(null)).toBe("borrowExisting");
    expect(parseNativeDiscordTakeover(undefined)).toBe("borrowExisting");
    expect(parseNativeDiscordTakeover("quit")).toBe("borrowExisting");
    expect(parseNativeDiscordTakeover(true)).toBe("borrowExisting");
    expect(parseNativeDiscordTakeover("quitAndRelaunch")).toBe("quitAndRelaunch");
  });

  it.each(["existingSessionQuitRefused", "takeoverNotPermitted"])("preserves the bounded takeover failure reason %s", async (reason) => {
    const response = { id: "discord", status: "failed", reason, mode: "none", captureProtected: false };
    mocks.invoke.mockResolvedValueOnce(response);
    await expect(hostNativeAppWindow("discord", "existingSession", "quitAndRelaunch")).resolves.toEqual(response);
  });

  it("probes takeover consent read-only and rejects a non-boolean answer", async () => {
    mocks.invoke.mockResolvedValueOnce(true);
    await expect(nativeAppTakeoverRequiresConsent("discord")).resolves.toBe(true);
    expect(mocks.invoke).toHaveBeenCalledWith("native_app_takeover_requires_consent", { appId: "discord" });

    mocks.invoke.mockResolvedValueOnce(false);
    await expect(nativeAppTakeoverRequiresConsent("discord")).resolves.toBe(false);

    mocks.invoke.mockResolvedValueOnce("true");
    await expect(nativeAppTakeoverRequiresConsent("discord")).rejects.toThrow("invalid native takeover consent response");
