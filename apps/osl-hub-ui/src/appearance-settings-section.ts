...
export function appearanceSettingsContent(
  ...
  return `${legacyControlsOnly}<section class="appearance-theme"><h3 class="machine-fact">Theme</h3>${theme}</section><div data-settings-profile-mount>${settingsProfileBlockMarkup(profileDraft)}</div>${appearanceLivePreviewMarkup(profileDraft, accent)}`;
}
