...
export type GeneratedCapabilityLabel = "Ready" | "Placing only" | "Reading only" | "Opens the app" | "Not started";

export interface ServiceCapabilityFacts {
  placing: boolean;
  reading: boolean;
  opening: boolean;
...
/** Browser-preview equivalent of the Rust rule; native service labels arrive generated by Rust. */
export function generatedCapabilityLabel(facts: ServiceCapabilityFacts): GeneratedCapabilityLabel {
...
export function homeAppsFromServices(services: readonly LinkedService[]): HomeAppCatalogEntry[] {
  ...
  return definitions.map((definition) => {
    ...
    return {
      ...
      generatedLabel: service?.generatedLabel ?? generatedCapabilityLabel({ placing: false, reading: false, opening: false, realTwoPersonProtectedMessaging: false }),
...
export function parseLinkedServices(raw: unknown): LinkedService[] | null {
  ...
  for (const candidate of raw) {
    if (!isExactRecord(candidate, ["id", "displayName", "sidebarGlyph", "sidebarOrder", "category", "launchState", "generatedLabel", "supportsNativePreview", "supportsProtectedPreview", "accounts"])) return null;
    ...
    if (!["Ready", "Placing only", "Reading only", "Opens the app", "Not started"].includes(String(candidate.generatedLabel))) return null;
    ...
    parsed.push({
      ...
      generatedLabel: candidate.generatedLabel as GeneratedCapabilityLabel,
...
function service(id: ServiceId, displayName: string, sidebarGlyph: string, sidebarOrder: number, category: ServiceCategory, launchState: LaunchState): LinkedService {
  const capabilityFacts: ServiceCapabilityFacts = id === "discord" || id === "telegram"
    ? { placing: true, reading: true, opening: true, realTwoPersonProtectedMessaging: false }
    : id === "whatsapp" || id === "signal"
      ? { placing: false, reading: true, opening: true, realTwoPersonProtectedMessaging: false }
      : { placing: false, reading: false, opening: true, realTwoPersonProtectedMessaging: false };
  const generatedLabel = generatedCapabilityLabel(capabilityFacts);
  return { id, displayName, sidebarGlyph, sidebarOrder, category, launchState, generatedLabel, supportsNativePreview: launchState === "available", supportsProtectedPreview: launchState === "available", accounts: [] };
}
