...
export function privacyDestinationContent(
  ...
  const policyCards = policyGroups.map(([name, detail, status]) => `<article class="privacy-policy-card">${statusTag(status)}<h3>${name}</h3><p>${detail}</p></article>`).join("");
  const toolRows = tools.map(([name, detail], index) => `<article class="setting-line privacy-tool-row"><span><strong>${name}</strong><small>${detail}</small></span>${statusTag(index === 0 ? "Available" : state.proActive ? "Pro planned" : "Pro")}</article>`).join("");
