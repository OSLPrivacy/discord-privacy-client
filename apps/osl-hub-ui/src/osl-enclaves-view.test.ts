...
describe("Enclave honest states", () => {
  it("ships the Enclaves state view through the main server route", () => {
    ...
    expect(serversView).toContain("oslEnclavesSurfaceMarkup({ statusTag })");
  });

  it("keeps a queued send pending while offline", () => {
    const markup = oslEnclaveStateMarkup({ offline: true, queuedSends: 1 });

    expect(markup).toContain("osl-enclave-state--offline");
    expect(markup).toContain("1 message is waiting to send when you're back online.");
    expect(markup).not.toContain("delivered");
  });

  it("separates every outstanding Enclave condition from a completed state", () => {
    const markup = oslEnclaveStateMarkup({
      staleRoster: true,
