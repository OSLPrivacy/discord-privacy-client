...
import { attachmentPickerLimitMarkup } from "./attachment-picker-limit";
...
function chooseAppsOnboardingContent(): string {
  ...
  const choices = (items: HomeAppCatalogEntry[], label: string) => items.length
    ? `<div class="onboarding-app-grid onboarding-app-choices" role="group" aria-label="${label}">${items.map((app) => {
      ...
      const action = available ? `data-onboarding-app-choice="${app.id}" aria-pressed="${selected}"` : `data-onboarding-app-not-built="${app.id}"`;
...
function bindOnboarding(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-onboarding-app-not-built]").forEach((button) => button.addEventListener("click", () => {
    ...
    const app = homeAppsFromServices(services).find((a) => a.id === appId);
    if (app) showToast(`${app.displayName} isn't built yet`);
  }));
...
function serviceContent(): string {
  ...
  // TASK 0853 put the generated capability, sentence and next action on this
  // route. TASK 0856 makes them the route: a status page that states the real
  // capability, the plain limits, one action that works today, and Back to
  // Home. Same controls as the card it replaces — `#embedded-service-setup`
  // still opens the profile, `#burn-button` still opens Burn, and
  // `#native-app-back` keeps its handler and finally carries the label that
...
function privacySettingsContent(): string {
  ...
  return `<h2>Scrub</h2><p class="scrub-local-promise"><strong>Your messages never leave this device.</strong> Every scan and review stays local.</p>${gatedRoute}${scanControls}${scrubCategoryChooserMarkup()}${privacyScanResultsMarkup()}${autoScrubAssistantMarkup(proActive)}<details class="safety-disclosure scrub-safety"><summary>Before deleting anything</summary><div><p><strong>Use at your own risk.</strong> Suggestions can be wrong. Check every message first.</p><p>Deletion can be irreversible. Here is each limit, plainly.</p><ul class="scrub-threat-limits"><li data-scrub-limit="copies"><strong>Copies.</strong> Scrub cannot undo copies. A message someone forwarded, quoted, exported, or saved stays where they put it.</li><li data-scrub-limit="screenshots"><strong>Screenshots.</strong> Scrub cannot undo screenshots. Anything already captured on another screen stays captured.</li><li data-scrub-limit="service-records"><strong>Service records.</strong> Scrub cannot undo service records. Logs, backups, and moderation copies a service keeps are out of your reach and out of ours.</li><li data-scrub-limit="service-permission"><strong>Service permission.</strong> Scrub cannot guarantee service permission. A service is free to refuse, rate-limit, suspend, or ban an account over scanning or deleting, and OSL holds no agreement with any service that says otherwise.</li></ul><p>Only a service recheck can verify removal within its stated coverage, and no service owes you that recheck.</p><p data-scrub-limit="approved-account"><strong>Approved account.</strong> Point Scrub only at an account you hold and are approved to use. That approval is yours to hold and yours to answer for: OSL cannot check it, no service has agreed to let OSL scan or delete on your behalf, and using an account you are not approved for is on you.</p><p>Automatic deletion is unavailable in this build until the native one-shot reviewed-consent capability is available. Connect IMAP for read-only verification. IMAP reaches a mailbox only where your mail host still runs it for your account and you have switched it on; where it is off, nothing here can confirm a deletion.</p><p>This build only gives manual directions. It does not delete app messages. You are responsible. Check the original app and delete each message yourself.</p></div></details><details class="privacy-technical settings-disclosure"><summary>Privacy and technical details</summary><div class="setting-line"><span>Default key expiry</span><strong>${timer}</strong></div><div class="setting-line"><span>Remote app access</span><strong>Blocked</strong></div><div class="setting-line"><span><strong>Windows capture resistance</strong><small>Always applied to OSL’s own window. Cameras, malware, and modified recipients can still capture content.</small></span><strong>${screenshotProtectionEnabled ? "Active" : "Unavailable"}</strong></div></details>`;
}
