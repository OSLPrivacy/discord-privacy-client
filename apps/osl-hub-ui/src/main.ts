...
function mailComposerEncryptionScope(app: HomeAppCatalogEntry | null): string {
  ...
  return `<aside class="mail-composer-encryption-scope" data-mail-composer-encryption-scope="${app.id}" role="note" aria-label="Email protection scope"><strong>Before you send</strong><small>${escapeHtml(app.displayName)} protects this app account. Ordinary external email is outside OSL Chat protection. Use OSL Chat for verified friends.</small></aside>`;
}
