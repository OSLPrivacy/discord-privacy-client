...
export function whitelistRosterPersonMarkup(
  ...
  const truncated = hiddenScopeCount > 0 || person.whitelistedScopesTruncated
    ? `<small class="whitelist-roster-truncated"><span class="machine-fact">${hiddenScopeCount > 0 ? `${hiddenScopeCount} more approved ${hiddenScopeCount === 1 ? "chat is" : "chats are"}` : "More approved chats are"}</span> stored locally and not listed here.</small>`
    : "";
