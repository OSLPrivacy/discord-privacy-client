...
function readyMarkup(model: PeerProtectedSheetModel): string {
  ...
  const write = `<form id="peer-protect-form" class="local-protected-form">
      ...
      <textarea id="peer-protected-draft" maxlength="1000" rows="5" autocomplete="off" spellcheck="true" placeholder="Write privately">${escapeHtml(model.draft)}</textarea>
      <label class="peer-ttl"><span>Relay copy expires after</span><select id="peer-protected-ttl">${ttlOptions}</select><small>Copies already opened remain.</small></label>
  ...
  const open = `<form id="peer-open-form" class="local-protected-form">
      ...
      <textarea id="peer-cover-input" maxlength="262144" rows="6" autocomplete="off" spellcheck="false" placeholder="Paste here yourself"></textarea>
