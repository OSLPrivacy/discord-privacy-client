...
describe("per-account opening contract", () => {
  ...
  it("forces a picker when any account overrides the provider-wide default", () => {
    const selected = slice("providerWideInstalledNativeApp", "nativeSessionModeForApp");
  ...
  it("drops stale account overrides before saving them", () => {
    const persist = slice("persistDetectedAccountChoices", "detectedAppsContent");
  ...
  it("never lets an exact isolated account click fall through to native reuse", () => {
    ...
    expect(open).toContain("const nativeIntent = accountId ? undefined : selectedNativeAppIntent(app.id)");
    ...
    expect(setup).toContain("const nativeIntent = forceNewProfile ? undefined");
  });
});
