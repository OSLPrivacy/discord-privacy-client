...
export interface OslEnclavesRegionModel {
  ...
  readonly supplementaryChannelListMarkup?: string;
...
export function oslEnclavesFiveRegionSurface(model: OslEnclavesRegionModel): string {
  lastModel = model;
  return `<main class="content-viewport osl-enclaves-page"><h1 id="route-heading" class="sr-only" tabindex="-1">OSL Enclaves</h1>${appRailMarkup()}<div class="enclv-dynamic" data-enclave-root>${dynamicMarkup(model)}</div>${model.supplementaryChannelListMarkup ?? ""}${model.roleEditorMarkup ?? ""}</main>`;
}
