...
describe("browser account model", () => {
  ...
  it("asks once after an import, then remembers the browser account mode", () => {
    ...
    expect(opening).toContain("selectedBrowserHasImportReceipt() && !browserSessionModeConfirmed");
    expect(opening).toContain("openServiceRoute(service, app.provider, app.id, true)");
    expect(binding).toContain("browserSessionModeConfirmed = true");
    expect(binding).toContain('localStorage.setItem(browserSessionModeConfirmedStorageKey, "true")');
  });
