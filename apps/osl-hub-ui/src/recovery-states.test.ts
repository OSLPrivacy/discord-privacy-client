...
const CONTRACT_ROWS = [
  ...
  ["password-recovery-capable", "Forgotten password with a recovery-capable marker and password recovery phrase", "A new main password; the local store is re-keyed before it is opened.", "Nothing caused by the password reset itself."],
  ...
  ["burn-credential", "Burn credential used", "The identity only if its identity phrase was saved, and then only as keys.", "OSL's local account core, local history and local device state; it does not erase recipients' already opened messages or the user's original third-party account data."],
