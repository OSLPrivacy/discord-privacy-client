...
export interface OslChatFriend {
  ...
  /** True only when both people have saved an allowance for the other. */
  verificationTwoWay?: boolean;
  /** The independently checked answer for one named build-proof file. */
...
/** The build mark. Mutual allowance cannot create this mark. */
...
function friendRow(friend: OslChatFriend, activePersonId: string | null, busy: boolean): string {
  ...
  return `<div class="osl-chat-friend${active ? " is-active" : ""}${friend.keyChanged ? " is-key-changed" : ""}" data-person-id="${escapeHtml(friend.personId)}">
    <button class="osl-chat-friend-open" type="button" data-osl-chat-open="${escapeHtml(friend.personId)}" ${active ? 'aria-current="true"' : ""} ${busy || !friend.verified ? 'disabled aria-disabled="true"' : ""}>
      ${avatar(friend.nickname)}<span class="osl-chat-friend-copy"><strong>${escapeHtml(friend.nickname)}${friendIdentityMarks(friend)}</strong>${friendPreview(friend)}</span>${friend.keyChanged ? '<span class="osl-chat-key-changed-triangle" role="img" aria-label="Safety number changed">⚠</span>' : `<span class="osl-chat-kind" title="Direct message">${chatIcon}</span>`}
    </button>
