...
export interface OslChatFriend {
  ...
  /**
   * True only when both people have saved an allowance for the other. This is
   * consent state, not evidence about either person's installed build.
   */
  verificationTwoWay?: boolean;
  /**
   * The independently checked answer for one named build-proof file. The UI
   * recognizes only the checker's exact positive answers; every other answer
   * fails closed to a visible "Cannot tell" badge.
   */
  buildProof?: OslChatFriendBuildProof;
}

export interface OslChatFriendBuildProof {
  proofName: string;
  answer: string;
}
...
const buildUnknownIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3 5 6v5c0 4.6 2.8 8.1 7 10 4.2-1.9 7-5.4 7-10V6l-7-3Z"/><path d="M9.8 9.4A2.4 2.4 0 0 1 12 8c1.4 0 2.5.9 2.5 2.1 0 1.7-2.5 1.8-2.5 3.5M12 17h.01"/></svg>';

/** The mutual-allowance mark. It never consults build-proof state. */
export function oslVerificationTickMarkup(twoWay: boolean | undefined): string {
  if (twoWay !== true) return "";
  return `<span class="osl-consent-mark osl-verification-tick" data-osl-consent-mark="two-way" data-osl-verification-tick="visible" role="img" aria-label="Both people allow each other" title="Both people allow each other">${consentTickIcon}</span>`;
}

/**
 * The build mark. A positive mark requires both a non-empty proof name and the
 * checker answer `unmodified`; mutual allowance cannot create this mark.
 */
export function oslBuildProofMarkMarkup(
  proof: OslChatFriendBuildProof | undefined,
): string {
  if (!proof) return "";
  const proofName = proof.proofName.trim();
  const answer = proof.answer.trim();
  if (proofName && answer === "unmodified") {
    const accessible = `Unmodified build — ${proofName}`;
    return `<span class="osl-build-proof-mark is-unmodified" data-osl-build-mark="unmodified" data-osl-build-proof-name="${escapeHtml(proofName)}" role="img" aria-label="${escapeHtml(accessible)}" title="${escapeHtml(accessible)}">${buildTickIcon}<span>Unmodified build</span></span>`;
  }
  if (proofName && answer === "modified") {
    const accessible = `Modified build — ${proofName}`;
    return `<span class="osl-build-proof-mark is-modified" data-osl-build-mark="modified" data-osl-build-proof-name="${escapeHtml(proofName)}" role="img" aria-label="${escapeHtml(accessible)}" title="${escapeHtml(accessible)}">${buildUnknownIcon}<span>Modified build</span></span>`;
  }

  const reason = answer.startsWith("OSL cannot check this person's app")
    ? answer
    : "OSL cannot check this person's app.";
  const accessible = proofName
    ? `Cannot tell whether the build is unmodified — ${proofName}. ${reason}`
    : `Cannot tell whether the build is unmodified. ${reason}`;
  return `<span class="osl-build-proof-mark is-cannot-tell" data-osl-build-mark="cannot-tell"${proofName ? ` data-osl-build-proof-name="${escapeHtml(proofName)}"` : ""} role="img" aria-label="${escapeHtml(accessible)}" title="${escapeHtml(accessible)}">${buildUnknownIcon}<span>Cannot tell</span></span>`;
}

function friendIdentityMarks(friend: OslChatFriend): string {
  return `${oslVerificationTickMarkup(friend.verificationTwoWay)}${oslBuildProofMarkMarkup(friend.buildProof)}`;
}
...
function friendRow(friend: OslChatFriend, activePersonId: string | null, busy: boolean): string {
  ...
  return `<div class="osl-chat-friend${active ? " is-active" : ""}" data-person-id="${escapeHtml(friend.personId)}">
    <button class="osl-chat-friend-open" type="button" data-osl-chat-open="${escapeHtml(friend.personId)}" ${active ? 'aria-current="true"' : ""} ${busy || !friend.verified ? 'disabled aria-disabled="true"' : ""}>
      ${avatar(friend.nickname)}<span class="osl-chat-friend-copy"><strong>${escapeHtml(friend.nickname)}${friendIdentityMarks(friend)}</strong>${friendPreview(friend)}</span><span class="osl-chat-kind" title="Direct message">${chatIcon}</span>
    </button>
