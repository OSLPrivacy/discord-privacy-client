...
describe("native Discord protected overlay routing", () => {
  ...
  it("aligns Discord without focusing it, and fails closed, before requesting the overlay", () => {
    ...
    // Aligned, never focused. This used to call focusActiveNativeCompanion(),
    // which brings the borrowed Discord window to the foreground -- handing
    // Discord the keyboard at the exact moment the operator is about to type
    // into OSL's protected composer, so the keystrokes in between went to
    // Discord in the clear. The alignment is still required and still fails
    // closed, because the composer is positioned against that rectangle.
    expect(openProtection).toMatch(
      /await alignActiveNativeCompanion\(\)[\s\S]*?Protection stopped: the Discord window could not be aligned safely\.[\s\S]*?await setNativeDiscordProtectedOverlayOpen/,
    );
    expect(openProtection).not.toMatch(/await focusActiveNativeCompanion\(\)/u);
    // And the helper it now calls must not take the foreground itself.
    const align = source.slice(
      source.indexOf("async function alignActiveNativeCompanion"),
      source.indexOf("async function reopenActiveNativeCompanion"),
    );
    expect(align).toContain("resizeNativeAppWindow()");
    expect(align).not.toContain("focusNativeAppWindow");
  });
  ...
  it("keeps overlay focus reclaim single-owner and rate bounded", () => {
    expect(nativeOverlay).not.toContain("start_throttled_focus_reclaim");
    expect(nativeOverlay).toMatch(
      /focus_reclaim_attempt_due\(\s*focus_reclaim_pending,\s*last_focus_reclaim_attempt\.elapsed\(\),?\s*\)/,
    );
    expect(nativeOverlay).toContain("elapsed >= Duration::from_millis(250)");
  });
  ...
  it("surfaces the backend's own refusal alongside the friendly sentence", () => {
    ...
    // The friendly sentence is kept and only appended to, never replaced.
    expect(helper).toContain("return sentence;");
    expect(helper).toContain("`${sentence}: ${failure.message}`");
    // Scoped to the command AND bounded by age, so a refusal from an earlier
    // unrelated attempt cannot be pinned onto this one.
    expect(helper).toContain("lastBackendFailure(command)");
    expect(helper).toContain("Date.now() - failure.at > BACKEND_REASON_FRESHNESS_MS");
    // The three sites, each naming the command whose journal entry it reports.
    expect(source).toContain(
      'showToast(withBackendReason("Protection could not open safely", "set_local_protected_sheet_open"));',
    );
    expect(source).toContain('"set_native_discord_protected_overlay_open",\n          ));');
    expect(source).toContain(
      'showToast(withBackendReason("OSL Chat could not open", "activate_osl_chat_context"));',
    );
    // It adds nothing of its own: no site may invent a reason the backend did
    // not give, and the deliberately uniform send refusal is not one of them.
    expect(source).not.toContain("withBackendReason(\"OSL could not deliver");
  });
});

describe("the backend-failure journal semantics main.ts relies on", () => {
  it("scopes to one command and timestamps every entry", async () => {
    const { clearBackendFailures, lastBackendFailure, recordBackendFailure } =
      await import("./backend-failure");
    clearBackendFailures();
    expect(lastBackendFailure("set_local_protected_sheet_open")).toBeNull();

    const before = Date.now();
    recordBackendFailure("activate_osl_chat_context", "the activated context did not match");
    recordBackendFailure(
      "set_local_protected_sheet_open",
      "The verified native composer surface is unavailable",
    );

    // Newest-for-that-command, not merely newest: the OSL Chat sentence must
    // never report the calibration refusal that happened after it.
    expect(lastBackendFailure("activate_osl_chat_context")?.message)
      .toBe("the activated context did not match");
    expect(lastBackendFailure("set_local_protected_sheet_open")?.message)
      .toBe("The verified native composer surface is unavailable");
    // And a command that did not fail contributes nothing to append.
    expect(lastBackendFailure("set_native_discord_protected_overlay_open")).toBeNull();
    // The freshness bound main.ts applies needs a real timestamp to work.
    const at = lastBackendFailure("activate_osl_chat_context")?.at ?? 0;
