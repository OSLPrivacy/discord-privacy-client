...
function signalReceipt(raw: unknown, expectedStatus: SignalQaNativeReceipt["status"]): SignalQaNativeReceipt {
  ...
  return candidate as unknown as SignalQaNativeReceipt;
}
...
export const signalQaNativeDependencies: SignalQaShellDependencies = {
  ...
  async hostNativeAppWindow(appId, mode) {
    ...
    return signalReceipt(await invoke<unknown>("host_native_app_window", { appId }), "existingSession");
  },
