...
export interface LocalScrubIndexAdapters {
  ...
  resume: typeof resumeScrubIndex;
...
export async function persistLocalScrubExport(
  ...
  if (status?.phase === "complete") {
    const matching = await adapters.initialize(initialization);
    if (matching.importId !== status.importId || matching.phase !== "complete") {
      throw new Error("The completed Scrub index could not be matched safely");
    }
    await adapters.cancel(matching.importId);
    status = await adapters.initialize(initialization);
  }
  if (!status) {
    status = await adapters.initialize(initialization);
  } else if (status.phase === "paused") {
    status = await adapters.resume(status.importId);
  }
