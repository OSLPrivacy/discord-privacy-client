...
export interface ScrubSettingsSectionState {
  ...
  /**
   * Coach-tip note for the explicit local scan control. It is rendered beside
   * the control, only in the scan step, and never wraps or replaces it, so a
   * dismissed tip cannot take the control away.
   */
  scanCoachTip: string;
}
