...
export function parseNativeDiscordOverlayState(value: unknown): NativeDiscordOverlayState | null {
  if (!exactRecord(value, ["active", "friendLabel", "scopeApproved", "ttlSeconds", "decryptDisplayEnabled", "viewOnceEnabled", "attachmentsEnabled", "discordMarkerAvailable"])) return null;
  if (value.active !== true || value.scopeApproved !== true || !boundedVisible(value.friendLabel, 80)
    ...
    || typeof value.attachmentsEnabled !== "boolean" || typeof value.discordMarkerAvailable !== "boolean") return null;
