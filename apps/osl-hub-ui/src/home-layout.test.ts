...
describe("home interaction regressions", () => {
  ...
  it("preserves explicit native intent and never silently falls back to the web", () => {
    ...
    expect(source).toContain("hostNativeAppWindow(appId, requestedMode, discordTakeover)");
    ...
    expect(source).toContain("const hostDeadlineMs = appId === \"discord\" && requestedMode === \"dedicated\"");
    expect(source).toContain("discordQaExistingSession");
