...
export function onboardingPasswordRoleContent({ role, configured, passwordEyeIcon, statusTag }: PasswordRoleContentOptions): string {
  ...
  // The submit lives in the step's shared action row rather than inside the
  // form card, so Back, the primary action and the "Not now" escape hatch land
  // in the same place here as on every other step. It stays this form's submit
  // through the HTML form-owner attribute, so Enter in a field still submits.
  const formId = `setup-${role}-form`;
  return `<h1 id="route-heading" tabindex="-1">${title}</h1><p class="compact-lead onboarding-centered-copy">${detail}</p><form id="${formId}" class="setup-surface password-form onboarding-role-form" data-onboarding-password-role="${role}" data-onboarding-password-next="${next}" novalidate><label for="setup-${role}-current">Current password</label><div class="password-input-row"><input id="setup-${role}-current" name="current" type="password" minlength="6" maxlength="128" autocomplete="current-password" required/><button class="password-eye" type="button" data-password-toggle="setup-${role}-current" aria-label="Show current password">${passwordEyeIcon()}</button></div><label for="setup-${role}-alternate">New ${stealth ? "stealth" : "burn"} password</label><div class="password-input-row"><input id="setup-${role}-alternate" name="alternate" type="password" minlength="6" maxlength="128" autocomplete="new-password" required/><button class="password-eye" type="button" data-password-toggle="setup-${role}-alternate" aria-label="Show new password">${passwordEyeIcon()}</button></div><label for="setup-${role}-confirm">Confirm</label><div class="password-input-row"><input id="setup-${role}-confirm" name="confirm" type="password" minlength="6" maxlength="128" autocomplete="new-password" required/><button class="password-eye" type="button" data-password-toggle="setup-${role}-confirm" aria-label="Show password confirmation">${passwordEyeIcon()}</button></div>${burnConfirmation}<p class="unlock-error" data-onboarding-role-error role="alert"></p></form><div class="setup-footer onboarding-actions"><button class="button primary" type="submit" form="${formId}" data-onboarding-role-submit disabled>Set password</button><button class="text-button onboarding-role-skip" type="button" data-skip-onboarding-password-role="${next}">Not now</button></div>`;
}
