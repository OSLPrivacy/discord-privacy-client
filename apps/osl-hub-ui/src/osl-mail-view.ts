...
export function oslMailViewMarkup(model: OslMailViewModel): string {
  ...
  const confirmations = `${model.deleteReceipt ? confirmation("Server deletion confirmed", model.deleteReceipt.receiptSha256) : ""}${model.burnReceipt ? confirmation("Mailbox burn confirmed", model.burnReceipt.receiptSha256) : ""}`;
