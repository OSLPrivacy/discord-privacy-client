...
describe("Discord protected transcript", () => {
  ...
  it("supports theme, density, and bounded zoom inputs", () => {
    ...
    expect(normalizeDiscordTranscriptPreferences({ theme: "discord-light", density: "cozy", zoom: 0.2 }).zoom)
      .toBe(0.75);
    expect(normalizeDiscordTranscriptPreferences({ theme: "discord-midnight", density: "compact", zoom: 1 }))
      .toEqual({ theme: "discord-midnight", density: "compact", zoom: 1 });
    expect(normalizeDiscordTranscriptPreferences({ theme: "discord-ash", density: "cozy", zoom: 1 }))
      .toEqual({ theme: "discord-ash", density: "cozy", zoom: 1 });
  });
  ...
  it("is integrated as the central overlay viewport above the existing composer", () => {
    ...
    // The composer's cyan ring is the lock-engaged indicator and must be present.
