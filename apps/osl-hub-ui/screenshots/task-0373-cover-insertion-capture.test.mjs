...
function assertFixedGateFixture() {
  ...
  for (const value of [
    ...
    assert.match(source, new RegExp(escapeRegExp(value), "u"));
  }
}
...
test("TASK 0373 captures the fixed Choose cover insertion screen", async () => {
  ...
  try {
    ...
    await page.navigate(`${url}screenshots/task-0373-cover-insertion-fixture.html`, { timeoutMs: 30_000 });
    await page.send("Accessibility.enable");

    await evaluateValue(page, `new Promise((resolve, reject) => {
      const deadline = Date.now() + 10000;
      const tick = () => {
        if (document.querySelector("#route-heading")?.textContent?.trim() === "Choose cover insertion"
          && document.querySelector("#continue-cover-draft")
          && document.querySelector("#onboarding-back")
          && document.querySelectorAll('input[name="cover-mode"]').length === 2) {
          requestAnimationFrame(() => requestAnimationFrame(resolve));
          return;
        }
        if (Date.now() > deadline) {
          reject(new Error("Choose cover insertion screen did not render"));
          return;
        }
        setTimeout(tick, 25);
      };
    ...
    const screen = await evaluateValue(page, `(() => {
      ...
      const controls = [...document.querySelectorAll("button, input")]
        .map((element) => ({
          ...
          label: element.closest("label")?.innerText.replace(/\\s+/gu, " ").trim() || "",
    ...
    const treeText = [
      screen.title,
      screen.text,
      screen.controls.map((control) => `${control.label} ${control.ariaLabel} ${control.text} ${control.value}`).join("\n"),
      accessibilityTreeText(ax.nodes ?? []),
    ].join("\n");
    const normalizedTree = treeText.toLowerCase();

    for (const required of REQUIRED) {
      assert.match(normalizedTree, new RegExp(escapeRegExp(required.toLowerCase()), "u"));
    }

    const coverChoices = screen.controls
      .filter((control) => control.tag === "input" && control.type === "radio" && control.value)
      .map((control) => `${control.value}:${control.label}`);
    assert.deepEqual(coverChoices, [
      "insert-on-send:Insert on send FREE Looks good ↵",
      "type-naturally:Type naturally PRO Looks good",
    ]);
