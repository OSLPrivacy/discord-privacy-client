...
(function () {
    ...
    // Probe-5 channel-sidebar lock: replace Discord's # icon on the
    // CURRENTLY-SELECTED server channel with the same tri-state OSL
    // lock the channel header uses. Click toggles
    // server-header-whitelist (same as the header lock). Only the
    // selected channel is replaced -- non-selected channel rows keep
    // their normal # icon.
    if (!window.__oslSidebarLockCssInstalled) {
        try {
            const _s = document.createElement("style");
            _s.textContent =
                // Hide Discord's icon ONLY on the selected channel,
                // ONLY when we've actually injected our lock alongside
                // it (the :has(...) gate). This way the # stays put
                // until we're ready to replace it -- no flicker, no
                // hide-without-replacement.
                "li[class*='modeSelected_']:has([" +
                SIDEBAR_LOCK_DATA_ATTR +
                "='1']) [class*='icon_']:not([" +
                SIDEBAR_LOCK_DATA_ATTR +
                "]){display:none !important;}" +
                // Our lock icon is hidden when its containing <li> is
                // NOT modeSelected (defensive cleanup -- the JS sweep
                // also removes orphaned locks, but the CSS rule
                // hides them instantly while the sweep tick catches
                // up).
                "li:not([class*='modeSelected_']) > * [" +
                SIDEBAR_LOCK_DATA_ATTR +
                "='1']{display:none !important;}";
            (document.head || document.documentElement).appendChild(_s);
            window.__oslSidebarLockCssInstalled = true;
        } catch (_) {}
    }

    // Probe-5: install / refresh the sidebar channel lock on the
    // currently-selected channel. Called every periodic-sweep tick;
    // idempotent (skips if already installed under the same scope).
    function oslSweepSidebarChannelLock() {
        const selected = document.querySelector(
            "li[class*='modeSelected_']"
        );
        if (!selected) {
            // No selected channel visible (DM view, settings open,
            // etc.). Clean up any orphan locks.
        ...
        // If our lock is already present in the selected li, just
        // refresh state from the existing scope. Otherwise inject.
        let lock = selected.querySelector(
            "[" + SIDEBAR_LOCK_DATA_ATTR + "='1']"
        );
        // Remove any orphans (lock present in a non-selected li from
        // a prior selection -- can happen if Discord just swapped
        // class without re-mounting the elements).
        const allLocks = document.querySelectorAll(
            "[" + SIDEBAR_LOCK_DATA_ATTR + "='1']"
        );
        for (const o of allLocks) {
            if (o === lock) continue;
            try {
                o.remove();
            } catch (_) {}
        ...
        if (!lock) {
            // Find Discord's icon container inside the selected <li>.
            // The icon lives in [class*='icon_']; its parent is the
            // container we'll inject into.
            const iconEl = selected.querySelector("[class*='icon_']");
            ...
            lock.addEventListener("click", function (e) {
                // Reuse the existing header-icon handler. It reads
                // oslCurrentChannelContext() which derives the scope
                // from the URL -- same channel as our sidebar lock
                // since we're targeting the modeSelected (= current)
                // item.
                try {
                ...
                // After click, refresh local state so the lock
            ...
            // Insert before Discord's icon (which the CSS rule will
            // now hide because our lock's :has() gate matches).
            iconEl.parentElement.insertBefore(lock, iconEl);
        }
        // Refresh the lock state from the current scope. Reuses the
        // same Rust IPCs the header lock uses.
        try {
            oslRefreshSidebarLockState(lock);
        } catch (_) {}
    }

    // Probe-5: refresh the sidebar lock SVG + color to reflect the
    // current server-channel whitelist state. Mirrors the relevant
    // branch of oslRefreshHeaderState but writes to the sidebar lock
    // element instead of the header button.
    async function oslRefreshSidebarLockState(lock) {
        if (!lock) return;
        const ctx = oslCurrentChannelContext();
        if (!ctx || !ctx.channelId) return;
        const scopeInput = oslScopeForCurrentContext(ctx);
        if (!scopeInput || scopeInput.kind !== "server_channel") {
            // Sidebar lock is only meaningful for server channels
            // (the header lock handles DMs/GCs from the channel
            // header).
            return;
        }
        try {
            ...
            if (st.server_header) {
                ...
                title =
                    "Server-wide whitelist ON. Click to turn OFF.";
            } else if (st.channel) {
                state = "partial";
            ...
            // Network blip; leave the existing state.
        }
    }
    ...
    function recvPeriodicSweep() {
        try {
            // Probe-5 channel-sidebar lock sweep. Idempotent + cheap.
            try {
                oslSweepSidebarChannelLock();
            } catch (_) {}
