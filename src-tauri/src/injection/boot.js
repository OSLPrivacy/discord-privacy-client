...
(function () {
    ...
    function oslInstallPhase7c() {
        ...
        // 7d-FIX3: bootstrap-side self-entry repair handles the
        // common relaunch case (identity already has snowflake → fix
        // peer_map). The first-launch case (no snowflake on identity
        // yet) needs boot.js to pull the snowflake out of Discord
        // runtime + register it. We defer to a longer setTimeout so
        // Discord's React tree is well-populated by the time we read
        // it (the gear/account-burn injections at 500ms already
        // exercise the same surfaces).
        nativeSetTimeout(function () {
            try {
    ...
    /**
     * 7d-FIX3: extract the local user's Discord snowflake from the
     * page-runtime React tree. Tries multiple strategies in order;
     * returns null if none succeed.
     *
     * Strategy A: walk the React fiber on the user-area avatar
     * inside `panels__` (the bottom-left user surface). Discord
     * mounts the local user's UserSummary there; `memoizedProps`
     * along the parent chain usually exposes `user.id` /
     * `currentUser.id`.
     *
     * Strategy B: walk the fiber on the message-input section
     * if open; some Discord builds put the local user prop on the
     * channel header's parent (the same anchor we already use for
     * `oslCurrentChannelContext`).
     *
     * Snowflakes are validated as 17-20 digit numeric so a stray
     * channel-id or guild-id doesn't pose as a user id.
     */
    function oslExtractDiscordSnowflakeFromRuntime() {
        ...
        if (panel) {
            ...
            // Last-ditch on the panels__ root itself.
            const id = walk(fiberFor(panel), 30);
            if (id) return id;
        }

        // Strategy B: channel header anchor — same surface
        // oslCurrentChannelContext uses. Often carries `selfId`
    ...
    /**
     * 7d-FIX3: check whether Tauri-side identity has a registered
     * snowflake; if not, extract it from Discord runtime and
     * register via `osl_register_self_snowflake`. Runs once per
     * boot — `osl_get_self_user_id` is the cheap probe (returns
     * the snowflake on success or an error on missing self-entry).
     * Self-recovers silently when identity is already complete.
     */
    ...
    function oslEnsureSelfSnowflakeRegistered() {
        ...
        oslInvoke("osl_get_self_user_id", {}).then(function (r) {
            if (r.ok && typeof r.value === "string" && /^\d{17,20}$/.test(r.value)) {
                // Already registered — bootstrap repair (Rust side)
                // ensured peer_map matches. Nothing to do.
                return;
            }
            // Not registered yet. Try runtime extraction.
