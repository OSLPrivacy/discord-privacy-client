...
impl PoolEntry {
    pub fn new(
        capability: Vec<u8>,
        cover: String,
        context: CoverContextVersion,
        created_at_seconds: u64,
    ) -> Option<Self> {
        if capability.is_empty() || cover.is_empty() || cover.len() > 1024 {
            return None;
        }
        Some(Self {
            capability,
    ...
    pub fn into_capability(mut self) -> Vec<u8> {
        std::mem::take(&mut self.capability)
    }
}
...
impl CarrierPool {
    ...
    pub fn push(&mut self, entry: PoolEntry) -> Result<(), PoolEntry> {
        if self.entries.len() >= MAX_POOL_ENTRIES_PER_SCOPE {
            Err(entry)
        } else {
    ...
    pub fn discard_stale(
        ...
        self.entries.retain(|entry| {
            entry.context == context
                && now_seconds.saturating_sub(entry.created_at_seconds) <= max_age_seconds
        });
    }
    pub fn invalidate_for_context_change(&mut self) {
        self.entries.clear();
    }
    pub fn invalidate_for_scope_burn(&mut self) {
        self.entries.clear();
    }
}

impl Default for CarrierPool {
    fn default() -> Self {
        Self::new()
    }
}
