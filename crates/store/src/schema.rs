...
/// Durable history-copy state is deliberately separate from the command's
/// declared transition list.  These physical tables let SQLite's authorizer
/// independently observe every ledger, semantic, edge, visibility, and meter
/// access while the copied objects themselves land in the shipping tables.
const SCHEMA_HISTORY_COPY: &str = r#"
CREATE TABLE IF NOT EXISTS history_copy_operations (
    operation_bi BLOB PRIMARY KEY,
    request_commitment BLOB NOT NULL,
    state INTEGER NOT NULL CHECK (state IN (0, 1)),
    result_nonce BLOB,
    result_ciphertext BLOB,
    CHECK ((state = 0 AND result_nonce IS NULL AND result_ciphertext IS NULL) OR
           (state = 1 AND result_nonce IS NOT NULL AND result_ciphertext IS NOT NULL))
);

CREATE TABLE IF NOT EXISTS history_copy_semantics (
    mid_bi BLOB PRIMARY KEY,
    operation_bi BLOB NOT NULL,
    nonce BLOB NOT NULL,
    ciphertext BLOB NOT NULL,
    FOREIGN KEY (operation_bi) REFERENCES history_copy_operations(operation_bi)
);

CREATE TABLE IF NOT EXISTS history_copy_edges (
    edge_bi BLOB PRIMARY KEY,
    operation_bi BLOB NOT NULL,
    nonce BLOB NOT NULL,
    ciphertext BLOB NOT NULL,
    FOREIGN KEY (operation_bi) REFERENCES history_copy_operations(operation_bi)
...
CREATE TABLE IF NOT EXISTS history_copy_visibility (
    mid_bi BLOB PRIMARY KEY,
    operation_bi BLOB NOT NULL,
    nonce BLOB NOT NULL,
    ciphertext BLOB NOT NULL,
...
CREATE TABLE IF NOT EXISTS history_copy_meter (
    operation_bi BLOB PRIMARY KEY,
    nonce BLOB NOT NULL,
    ciphertext BLOB NOT NULL,
    FOREIGN KEY (operation_bi) REFERENCES history_copy_operations(operation_bi)
...
pub(crate) fn history_copy_schema_exists(conn: &Connection) -> Result<bool, StoreError> {
    for table in [
        "history_copy_operations",
        "history_copy_semantics",
        "history_copy_edges",
        "history_copy_visibility",
        "history_copy_meter",
    ] {
...
pub(crate) fn ensure_history_copy_schema(conn: &Connection) -> Result<(), StoreError> {
    conn.execute_batch(SCHEMA_HISTORY_COPY)?;
    Ok(())
}
