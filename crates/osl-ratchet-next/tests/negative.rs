...
fn a_restored_session_cannot_reuse_a_consumed_message_key() {
    ...
    let second = alice
        .encrypt(0, b"second skipped", &mut rng)
        .expect("second");
    let third = alice.encrypt(0, b"third skipped", &mut rng).expect("third");
    assert_eq!(
        bob.decrypt(&third, &mut rng)
            .expect("derive skipped keys")
            .plaintext,
        b"third skipped"
    );
    assert_eq!(
        bob.decrypt(&first, &mut rng)
            .expect("consume skipped key")
            .plaintext,
        b"first skipped"
    );
    ...
    let before_skipped_replay = restored
        .export_state()
        .expect("export restored skipped state");
