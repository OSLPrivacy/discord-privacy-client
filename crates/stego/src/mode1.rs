...
//
// Encodes a 20-byte carrier seed `P` as chat-like cover text with:
...
//     volumes).
//
// Payload layout: [P (20 B)] [detect_tag (4 B)] = 24 bytes = 192 bits.
...
/// Carrier seed bytes carried per prose token. `P` is a 160-bit seed, not a
/// cipher-store id or bearer capability.  The sender and receiver derive the
/// server-facing authority from it locally; it must never be sent to the
/// cipher store.
pub const TOKEN_ID_BYTES: usize = 20;

/// Detection tag bytes appended for receiver-side token detection.  This is
/// carrier-only data and is never transmitted to the cipher store.
pub const DETECT_TAG_BYTES: usize = 4;
/// Backwards-compatible name for the detection tag width.
pub const TOKEN_MAC_BYTES: usize = DETECT_TAG_BYTES;

/// Total bits in the encoded payload (`P || detect_tag`).
...
/// coincidentally in-vocabulary English. Public so wire-compatibility tests
/// can build a cover in an older carrier format without reimplementing the
/// HMAC; it derives only from inputs the caller already holds.
pub fn compute_token_tag(mac_key: &[u8], id: &[u8; TOKEN_ID_BYTES]) -> [u8; TOKEN_MAC_BYTES] {
...
/// Encode a 160-bit carrier seed as marker-free, chat-like cover text.
...
pub fn encode_token(
    ...
    // Bigram pivot: the prose cover is generated by the n=2
    // language-model arithmetic codec (see `crate::bigram`) instead
    // of the 16-template / 2-slot scheme.
    //
    // B0-06: the payload (`P || detect_tag` = 192 bits) is chunked so it
    // fits the arithmetic interval — before that it overflowed `PRECISION`
    // and every cover was emitted by the 6-bit / 64-word substitution
    // fallback, i.e. 32 words of word salad. That fallback is now
    // receive-only (`decode_token_legacy_wide`); nothing here emits it.
    //
    // `_cipher` is retained in the signature for API compatibility but is
...
///
/// Tries the chunked arithmetic codec first (current format); then the
/// pre-B0-06 6-bit / 64-word substitution carrier, so covers posted by peers
/// on the old build still decode during the rollout window; then the legacy
/// template decoder, so covers posted by pre-bigram-pivot peers do too.
///
/// Each fallback is gated by the same 32-bit detect tag as the primary path,
/// so trying three codecs costs three HMAC comparisons and adds at most
/// 3 × 2⁻³² to the false-positive rate.
pub fn decode_token(
    ...
    if let Some(id) = decode_token_legacy_wide(mac_key, msg) {
        return Some(id);
    }
...
/// re-encodes them to the 192-bit payload, splits `P || detect_tag`, and
/// verifies the HMAC.
fn decode_token_bigram(mac_key: &[u8], msg: &str) -> Option<[u8; TOKEN_ID_BYTES]> {
    ...
    decode_token_from_bits(mac_key, &words, &bits, CoverCodec::Arithmetic)
}

/// RECEIVE ONLY (B0-06). Pre-B0-06 senders emitted the 6-bit / 64-word
/// substitution carrier because the 192-bit payload overflowed the
/// arithmetic interval. Those covers are still on the wire in any live
/// conversation, so they must still decode. Nothing calls the matching
/// encoder on the send path — see `encode_token`.
fn decode_token_legacy_wide(mac_key: &[u8], msg: &str) -> Option<[u8; TOKEN_ID_BYTES]> {
    let words = crate::bigram::parse_words(msg)?;
    let bits = crate::bigram::legacy_wide_encode_words(&words, TOKEN_PAYLOAD_BITS);
    decode_token_from_bits(mac_key, &words, &bits, CoverCodec::LegacyWide)
}

/// Which word-stream codec produced `bits`. Selects the matching canonical
/// re-encoder for the anti-malleability check.
...
/// Shared tail of both word-stream decode paths: split `P || detect_tag`,
/// verify the tag, then require the word stream to be the unique canonical
/// encoding of the recovered payload.
fn decode_token_from_bits(
    mac_key: &[u8],
    words: &[usize],
    bits: &[bool],
    codec: CoverCodec,
    ...
    // Arithmetic decoding stops as soon as the payload bits are
    ...
    let canonical_words = match codec {
        CoverCodec::Arithmetic => {
            crate::bigram::arithmetic_decode_bits(&canonical_bits, TOKEN_PAYLOAD_BITS)
        }
        CoverCodec::LegacyWide => {
            crate::bigram::legacy_wide_decode_bits(&canonical_bits, TOKEN_PAYLOAD_BITS)
        }
    };
