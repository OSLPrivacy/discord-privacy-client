...
pub fn accept_b5_prekeys_and_persist_with_sealer(
    ...
    let initiator = osl_ratchet_next::peek_bootstrap_initiator_identity(wire)
        .map_err(RnError::from)?
        .ok_or_else(|| RnError::Protocol("not a bootstrap message".into()))?;
    let rn_opk_id = peek_bootstrap_one_time_prekey_id(wire)?;
    ...
    if let Some(rn_opk_id) = rn_opk_id.filter(|id| *id != 0) {
        let b5_opk_id = rn_opk_id_to_b5(rn_opk_id)?;
        if !prekeys.consume_opk(b5_opk_id) {
            return Err(RnError::PrekeyAdapter(
                "local one-time prekey was already consumed",
            ));
        }
    }
...
mod tests {
    ...
    fn rn_wire_in_enabled_follows_app_state_controlled_toggle() {
        ...
        assert!(
            !RN_WIRE_IN_ENABLED,
            "the compile-time RN fuse must remain disabled"
        );
        assert!(matches!(
            send_rn_for_state(&state, &store, &sealer, &peer, 73, b"off"),
            Err(RnError::WireInDisabled)
        ));

        state.set_rn_wire_in_enabled(true);
        let wire = send_rn_for_state(&state, &store, &sealer, &peer, 73, b"on")
            .expect("state-controlled gate enables the state-aware path");
        assert!(!wire.is_empty());
    }
