...
/// 8-hex-char random filename used as the upload name for a **sealed
/// cipher-store object**. Phase 8e: `.mp4` (was `.bin` post-8d-FIX2, was
/// `.png` through 8d).
///
/// This name never reaches Discord. It names the object OSL puts in its own
/// cipher store, where the extension is cosmetic. The name Discord sees comes
/// from [`spoiler_layout_decoy_filename`].
...
// ---------------------------------------------------------------------------
// The platform row: what OSL places on Discord for an image send
// ---------------------------------------------------------------------------

/// What a Discord reader learns from an OSL image send.
///
/// Deliberately not a reassuring sentence. The row is visible, the fact that a
/// file was attached is visible, and the decoy's shape discloses which of four
/// aspect buckets the protected image fell into. Nothing else crosses.
pub const IMAGE_ROW_METADATA_LEAKAGE_WARNING: &str =
    "A Discord reader sees a message with a spoilered image attached. The image OSL attaches is generated by OSL and contains none of your picture; its shape is rounded to one of four aspect buckets, so the row discloses roughly which of those four shapes your picture had, and nothing else about it.";

/// The plan for the file OSL asks the operator to attach to a Discord message.
///
/// Everything here is placed on Discord, so everything here is non-secret by
/// construction: a generated decoy, its dimensions, and a random name.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct PlatformImageRowPlan {
    /// Filename to give Discord. Starts with `SPOILER_`, which is Discord's own
    /// marker for "render this attachment blurred behind a reveal control".
    pub decoy_filename: String,
    /// The payload-free decoy image bytes.
    pub decoy_png: Vec<u8>,
    pub decoy_width: u32,
    pub decoy_height: u32,
    pub bucket: crate::decoy_mp4::LayoutDecoyBucket,
    pub metadata_warning: &'static str,
}

/// Why OSL will not place a native image row for this send.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlatformImageRowRefusal {
    /// The image's decoded dimensions were absent or degenerate. Without them
    /// there is no bucket, so there is no decoy that would make Discord lay out
    /// a rect of the right shape, so there is nothing honest to place.
    UndecodableDimensions,
}

/// Build the platform row plan for an image whose decoded pixel dimensions are
/// known.
///
/// The dimensions are consumed here and go no further: they choose a bucket,
/// and the bucket is the only thing [`crate::decoy_mp4::layout_decoy_png`] can
/// see. The caller never gets a decoy that varies with the picture.
pub fn plan_platform_image_row(
    image_width: u32,
    image_height: u32,
) -> Result<PlatformImageRowPlan, PlatformImageRowRefusal> {
    let bucket = crate::decoy_mp4::LayoutDecoyBucket::for_dimensions(image_width, image_height)
        .ok_or(PlatformImageRowRefusal::UndecodableDimensions)?;
    let (decoy_width, decoy_height) = bucket.dimensions();
    Ok(PlatformImageRowPlan {
        decoy_filename: spoiler_layout_decoy_filename(),
        decoy_png: crate::decoy_mp4::layout_decoy_png(bucket),
        decoy_width,
        decoy_height,
        bucket,
        metadata_warning: IMAGE_ROW_METADATA_LEAKAGE_WARNING,
    })
}

/// Discord's marker prefix for a spoilered attachment.
///
/// Chosen because it is Discord's own affordance rather than something OSL
/// invented: eye off, a reader sees the standard blurred-attachment treatment
/// with Discord's own reveal control and no OSL branding anywhere. It also
/// gives the blank decoy a reason to look blank.
pub const SPOILER_FILENAME_PREFIX: &str = "SPOILER_";

/// Random name for the decoy OSL attaches to a Discord message.
///
/// Carries no information: 4 random bytes, a fixed prefix, a fixed extension.
/// In particular it is not derived from the protected filename, the object id,
/// the conversation, or the fetch capability.
pub fn spoiler_layout_decoy_filename() -> String {
    let bytes = random::random_bytes(4);
    format!(
        "{SPOILER_FILENAME_PREFIX}{:02x}{:02x}{:02x}{:02x}.png",
        bytes[0], bytes[1], bytes[2], bytes[3]
    )
}
