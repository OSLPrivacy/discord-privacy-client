...
/// 7d-FIX3: ensure peer_map has a well-formed self-entry keyed
/// by the user's Discord snowflake, matching the loaded identity's
/// `user_id` and X25519 public key with `is_self = true`.
///
/// Called once from bootstrap.rs after `load_peer_map`, and from
/// `cmd_osl_register_self_snowflake` after identity gets a new
/// snowflake. Idempotent — a no-op if the entry already matches.
///
/// Returns a tuple (snowflake, repaired) for diagnostics. If
/// the identity has no snowflake yet, returns Err with a marker
/// string so the bootstrap caller can log appropriately.
///
/// This function mutates AppState IN MEMORY only. The caller is
/// responsible for persisting peer_map.json afterwards (via
/// `persist_peer_map_now`) when `repaired == true`. Splitting
/// avoids invoking `keystore::osl_config_dir()` from tests, which
/// would touch the user's real config dir.
pub fn verify_peer_map_self_entry(state: &AppState) -> Result<(String, bool), String> {
...

    {
        ...
        entry.osl_user_id = Some(osl_user_id.clone());
...
/// 7d-FIX3: convenience wrapper for production callers — runs
/// `verify_peer_map_self_entry` and persists peer_map.json if a
/// repair happened. Tests use the bare `verify_peer_map_self_entry`
/// and inspect the AppState directly.
...
/// 7d-FIX3: persist a Discord snowflake on the loaded identity and
/// repair the peer_map self-entry to match. Called from boot.js
/// the first time the runtime exposes the local user's snowflake
/// (typically immediately after `oslInstallPhase7c` install).
///
/// Validates snowflake format (17–20 digits). Rejects with a clear
/// error if the identity already has a different snowflake recorded
/// — that should never happen via the normal first-launch flow and
/// signals either a corrupted identity blob or an attempt to retag
/// against a different Discord account.
pub fn cmd_osl_register_self_snowflake(state: &AppState, snowflake: String) -> Result<(), String> {
    ...
    // Write identity first; then save to disk; then repair peer_map.
    // The lock scope is intentionally narrow — `run_verify` calls
    // `verify_peer_map_self_entry` which re-locks the identity, so
    // we must release this guard before that call.
    enum Step {
    ...
    let step = {
        ...
        if let Some(existing) = &id.discord_snowflake {
            ...
            // Clone the bits we need to save outside the lock.
    ...
    if let Err(e) = keystore::save_identity(&path, &to_save, sealer.as_ref()) {
        // Roll back the in-memory change if disk persist failed,
        // so callers can retry without lying about the durable
        // state.
        let mut guard = state.identity.lock().expect("identity mutex poisoned");
...
fn run_verify(state: &AppState) -> Result<(), String> {
    match verify_and_persist_peer_map_self_entry(state) {
        ...
        Err(reason) if reason == "no_discord_snowflake" => {
            // Shouldn't happen — we just set it above — but be defensive.
            eprintln!(
