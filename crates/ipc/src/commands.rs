...
mod rn_first_contact_command_tests {
    ...
    #[test]
    fn first_contact_prekey_bundle_signature_maps_peer_identity() {
    ...
    #[test]
    fn first_contact_verified_capabilities_feed_pre_send_select_wire_version() {
    ...
    #[test]
    fn first_contact_handshake_fetches_prekey_bundle_over_keyserver() {
    ...
    #[test]
    fn rn_session_store_matches_selected_sealer_metadata() {
        let state = AppState::new();
...
mod ratchet_policy_decision_tests {
    ...
    #[test]
    fn rn_dispatch_has_no_encrypt_v4_send_call_site_for_all_scope_kinds() {
        for scope in [
            crate::scope::Scope::dm("peer"),
            crate::scope::Scope::gc("group"),
            crate::scope::Scope::server_channel("server", "channel"),
    ...
    #[test]
    fn dm_send_routes_through_rn_wire_path_even_when_sender_keys_enabled() {
        assert_eq!(
            ratchet_policy_decision(&crate::scope::Scope::dm("peer"), 1, false),
            RatchetPolicyDecision::LegacyV3
        );
...
mod rn_inbound_unknown_tests {
    ...
    #[test]
    fn commands_rn_encrypt_decrypt_state_gated_round_trip() {
        assert!(
            !crate::wire_rn::RN_WIRE_IN_ENABLED,
    ...
    #[test]
    fn rn_decrypt_path_delayed_out_of_order_delivery_succeeds_from_fixture() {
        let fixture = rn_command_round_trip_fixture();
...
mod friend_request_acceptance_tests {
    ...
    fn cmd_osl_send_friend_request_creates_and_persists_pending_friend_request() {
        ...
        let duplicate = match persist_typed_friend_request_with_dir(
            &state,
            REQUESTER_DID.to_string(),
            request_for(scope),
            dir.path(),
    ...
    #[test]
    fn cmd_osl_send_friend_request_refuses_untrusted_then_persists_trusted_pending() {
        let dir = tempfile::tempdir().unwrap();
        let state = AppState::new();
...
mod burn_wrapped_key_coordination_tests {
    ...
    #[test]
    fn burn_coordination_posts_control_inbox_and_deletes_wrapped_keys() {
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let port = listener.local_addr().unwrap().port();
        let (tx, rx) = mpsc::channel();
        std::thread::spawn(move || {
...
mod identity_registration_prekey_onboarding_tests {
    ...
    #[test]
    fn identity_registration_onboarding_posts_initial_prekey_state() {
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let port = listener.local_addr().unwrap().port();
        let (tx, rx) = mpsc::channel();
