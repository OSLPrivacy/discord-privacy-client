...
mod password_policy_tests {
    ...
    fn run_inactivity_auto_lock_timer() {
        set_file_storage_key(None);
        let t0 = Instant::now();
        set_file_storage_key_after_main_password_unlock_at([0x42; 32], t0);

        assert!(!super::run_inactivity_auto_lock_timer_at(
            t0 + std::time::Duration::from_secs((INACTIVITY_AUTO_LOCK_SECONDS - 1) as u64)
        ));
        ...
        assert!(!super::run_inactivity_auto_lock_timer_at(
            t1 + std::time::Duration::from_secs((INACTIVITY_AUTO_LOCK_SECONDS - 1) as u64)
        ));
        assert_eq!(get_file_storage_key(), Some([0x42; 32]));

        assert!(super::run_inactivity_auto_lock_timer_at(
            t1 + std::time::Duration::from_secs(INACTIVITY_AUTO_LOCK_SECONDS as u64)
        ));
        assert_eq!(get_file_storage_key(), None);
