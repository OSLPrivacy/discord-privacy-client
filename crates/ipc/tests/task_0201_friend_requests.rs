...
use ipc::commands::{
    cmd_osl_block_friend_request, cmd_osl_create_friend_request,
    cmd_osl_decline_or_revoke_friend_request, cmd_osl_list_friend_requests, FriendRequestDecision,
};
...
fn decline_and_block_friend_request_commands_are_closed_local_actions() {
    ...
    let blocked =
        cmd_osl_block_friend_request(&state, PEER_DID.to_string(), (&scope).into(), None).unwrap();
