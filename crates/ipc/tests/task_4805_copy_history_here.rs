use ipc::commands::{
    ...
    cmd_osl_load_channel_history, COPY_MY_HISTORY_HERE_ACTION_LABEL,
...
fn state_with_store(dir: &std::path::Path, secret: &[u8; 32]) -> AppState {
    let state = AppState::new();
    let mut identity = identity_from_entropy([0x48; 16], ACCOUNT.to_owned());
...
fn marked_history_count(state: &AppState) -> usize {
    cmd_osl_load_channel_history(state, CHANNEL.to_owned(), Some(MESSAGE_COUNT as u32))
        .expect("history load")
        .into_iter()
        .filter(|row| row.plaintext.starts_with(MARKER))
        .count()
}
...
fn task_4805_new_device_stays_empty_until_copy_my_history_here() {
    ...
    let old_state = state_with_store(old_dir.path(), &old_secret);
    let new_state = state_with_store(new_dir.path(), &new_secret);
    let ids = message_ids();
    ...
    assert_eq!(new_count_after_sync_window, 0);
    assert_eq!(new_raw_text_occurrences_before_copy, 0);
