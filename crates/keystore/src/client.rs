...
/// The capability bitmap this client signs into new registration and rotation
/// requests. Keep this as a floor: a rollback must never silently advertise
/// less than a build that already shipped OSL-RN support.
pub const CLIENT_RN_CAPABILITIES: u32 = RN_CAP_WIRE_RN;
...
impl KeyServerClient {
    ...
    pub fn build_register_request(identity: &Identity) -> RegisterRequest {
        ...
        let rn_capabilities = CLIENT_RN_CAPABILITIES;
        let msg = reg_msg_with_capabilities(
            &identity.user_id,
            &ik_x25519_pub,
            &ik_ed25519_pub,
    ...
    pub fn fetch_identity_bundle(
        ...
        let pubkeys = self.fetch_pubkeys(&identity.user_id)?;
        let identity_bundle =
            crate::identity_bundle::IdentityBundle::from_identity_and_pubkeys_response(
                identity, &pubkeys,
            )
            .map_err(|_| Error::PeerBundleProofInvalid)?;
        let prekey_bundle = self.fetch_prekey_bundle(identity, &identity.user_id)?;
        identity_bundle
            .verify_full(
                &prekey_bundle,
...
mod tests {
    ...
    fn register_request_carries_nonzero_rn_capabilities() {
        ...
        let signature =
            crypto::ed25519::Signature::from_bytes(signature_bytes.as_slice().try_into().unwrap());
        let extended = reg_msg_with_capabilities(
            &request.user_id,
    ...
    fn fetch_identity_bundle_atomically_fetches_pubkeys_and_prekey_bundle() {
        ...
        let pubkeys =
            crate::identity_bundle::scheme1_pubkeys_response_for_test(&identity, 3, RN_CAP_WIRE_RN);
        ...
        let merged = client.fetch_identity_bundle(&identity, None).unwrap();
        ...
        let client = KeyServerClient::new(format!("http://127.0.0.1:{port}")).unwrap();
        assert!(matches!(
            client.fetch_identity_bundle(&identity, None),
