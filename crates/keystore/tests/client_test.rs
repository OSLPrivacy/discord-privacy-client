...
#[test]
fn compatible_control_inbox_uses_signed_sender_filter_without_capability_probe() {
    ...
    let (port, requests) =
        multi_response_server(vec![control_inbox_response(Some("peer-a"), &["peer-a"])]);
...
#[test]
fn compatible_control_inbox_refuses_legacy_shape_without_health_probe() {
    let _serial = active_account_test_lock().lock().unwrap();
    let nonce = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
    ...
    let identity = generate_identity("recipient".to_owned());
    let (port, requests) =
        multi_response_server(vec![legacy_control_inbox_response(&["peer-b", "peer-a"])]);
    let client = KeyServerClient::new(format!("http://127.0.0.1:{port}")).unwrap();
    assert!(matches!(
