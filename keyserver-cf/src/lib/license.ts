...
/// stores SHA-256(plaintext); plaintext is delivered once via
/// email at issuance.
///
...
function formatLicense(body14: string, checksum2: string): string {
...
/** Generate a fresh license key. Returns both the plaintext
 *  (for the issuance email) and its SHA-256 hash (for D1). */
export async function generateLicenseKey(hmacSecret: string): Promise<LicenseGenerated> {
  const body = generateRandomBody();
  const checksum = await computeChecksum(body, hmacSecret);
  const plaintext = formatLicense(body, checksum);
...
export function normalizeLicense(input: string): string | null {
  let s = input.toUpperCase().replace(/[\s-]/g, "");
  if (s.startsWith("OSL")) s = s.slice(3);
  ...
  return formatLicense(s.slice(0, BODY_LEN), s.slice(BODY_LEN));
}
...
export async function validateChecksum(
  ...
  const norm = normalizeLicense(plaintext);
  if (!norm) return false;
  // Strip prefix + dashes back to the 16 data chars.
  const data = norm.slice(PREFIX.length).replace(/-/g, "");
