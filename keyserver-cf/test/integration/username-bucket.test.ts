...
async function claim(name: string, id: string, pair: { publicKeyB64: string; signingKey: CryptoKey }) {
  ...
  return SELF.fetch("http://test/v1/usernames/claim", { method: "POST", headers: { "content-type": "application/json", "cf-connecting-ip": "203.0.113.90" }, body: JSON.stringify({ username: name, user_id: id, friend_code: invite.trim(), request_id, timestamp_ms, signature_b64, ...(await publicNameProofFields(SELF, id, pair.signingKey)) }) });
}
