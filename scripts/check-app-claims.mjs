...
const versionedPublicSupportMatrixFailure = publicSupportMatrixFailure;

function validateVersionedPublicSupportEntries(matrix) {
  const failures = [];
...
async function validateSupportMatrixEvidence(publicFragments = []) {
  const failures = [];
  const violations = [];
  let matrix;
...
async function validateVersionedPublicSupportRows(matrix) {
  const failures = [];
  const publicMatrix = matrix?.versioned_public_support_matrix;
  if (!publicMatrix || typeof publicMatrix !== "object" || Array.isArray(publicMatrix)) {
...
async function validateVersionedPublicSupportMatrix(matrix) {
  return [
    ...validateVersionedPublicSupportEntries(matrix),
    ...(await validateVersionedPublicSupportRows(matrix)),
  ];
}
...
async function scanRepository() {
  ...
  let supportMatrixObject = null;
  try {
    supportMatrixObject = JSON.parse(await readUtf8(SUPPORT_MATRIX_PATH));
  } catch {
    supportMatrixObject = null;
  }
  const publicClaimServices = supportMatrixPublicClaimServices(supportMatrixObject);
  const supportMatrixValidation = await validateSupportMatrixFile();
  const rows = [];
  const allViolations = [];
  ...
  const supportMatrixResult = await validateSupportMatrixEvidence(publicFragments);
  ...
  console.log(
    `Support matrix: chat-app evidence apps=${supportMatrixValidation.appCount}, source anchors=${supportMatrixValidation.anchorCount}, failures=${supportMatrixValidation.failures.length}`,
  );

  printViolations(allViolations);
  printFloorFailures(floorFailures);
  printSupportMatrixFailures(supportMatrixValidation.failures);

  return allViolations.length === 0
    && floorFailures.length === 0
    && supportMatrixValidation.failures.length === 0
    ? 0
    : 1;
}
...
async function runSelfTest() {
  ...
  const mutatedVersionedSupportMatrix = structuredClone(productionSupportMatrix);
  mutatedVersionedSupportMatrix.versioned_public_support_matrix.rows[0].public_claim_allowed = true;
  const versionedPublicSupportMatrixPassed =
    (await validateVersionedPublicSupportMatrix(productionSupportMatrix)).length === 0
    && (await validateVersionedPublicSupportMatrix(mutatedVersionedSupportMatrix)).some(
      (failure) => failure.name === "versioned_public_support_matrix",
    );
  ...
  const mutatedSourceSupportMatrix = JSON.parse(JSON.stringify(productionSupportMatrix));
  const firstAnchor = collectMatrixSourceAnchors(mutatedSourceSupportMatrix)[0];
  if (firstAnchor) {
  ...
  const supportMatrixSourceFailures = await validateSupportMatrixSources(
    collectMatrixSourceAnchors(mutatedSourceSupportMatrix),
  );
