...
$stagedRoot = 'C:\OSL\desktop-runner-staged'
$outRoot = 'C:\OSL\out'
...
function Save-StagedPayload([string]$SourcePath, [string]$Destination) {
  if ([string]::IsNullOrWhiteSpace($SourcePath)) {
    throw 'staged payload path is empty'
  }
  $SourcePath = $SourcePath -replace '\\{2,}', '\'
  $fromLeafName = $false
  if ($SourcePath -cnotmatch '^[A-Za-z]:') {
    if ($SourcePath -match '[\\/:]' -or $SourcePath -in @('.', '..')) {
      throw 'staged payload leaf name is invalid'
    }
    $SourcePath = Join-Path $stagedRoot $SourcePath
    $fromLeafName = $true
  }
  if ($fromLeafName) {
    if (-not (Test-Path -LiteralPath $SourcePath -PathType Leaf)) {
      throw 'staged payload file is missing'
    }
    Copy-Item -LiteralPath $SourcePath -Destination $Destination -Force
    return
  }
  $resolvedStagedRoot = [IO.Path]::GetFullPath($stagedRoot).TrimEnd(
    [IO.Path]::DirectorySeparatorChar,
    [IO.Path]::AltDirectorySeparatorChar
  )
  $resolvedSource = [IO.Path]::GetFullPath($SourcePath)
  if ((-not $fromLeafName) -and (-not $resolvedSource.StartsWith($resolvedStagedRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase))) {
    throw 'staged payload is outside the exact desktop-runner staging root'
  }
...
if ($StagedPayloadLeaf) {
  Save-StagedPayload $StagedPayloadLeaf $payloadPath
} elseif ($PayloadUri) {
  Save-ManagedIdentityArtifact $PayloadUri $payloadPath
} elseif ($PayloadPath) {
  Save-StagedPayload $PayloadPath $payloadPath
  ...
  throw 'desktop-runner requires PayloadUri, PayloadPath, PayloadGzipBase64, or PayloadBase64'
}
