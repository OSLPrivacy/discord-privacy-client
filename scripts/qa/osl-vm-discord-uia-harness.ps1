...
try {
  $result = switch ($Action) {
    ...
    'Inspect' {
      $main = Get-FreshMainSurface
      $state = Get-MainState
      $hostResult = Get-HostResult
      $overlayStatusMatches = @(Get-ControlMatches $main.Root `
        @([Windows.Automation.ControlType]::Text) @() 'discord-qa-overlay-state')
      $overlayStatus = if ($overlayStatusMatches.Count -eq 1) {
        [string]$overlayStatusMatches[0].Current.Name
      } else {
        ''
      }
      ...
      [pscustomobject]@{
        ...
        HostState = $hostResult.State; HostReason = $hostResult.Reason;
    ...
    'PrepareOverlay' {
      ...
      if (-not $ready.Satisfied) {
        $state = Get-MainState
        $qaComposer = Get-FreshControlCount 'Main' `
          @([Windows.Automation.ControlType]::Button) @() 'discord-qa-toggle-composer'
        if ($qaComposer -eq 1) {
          Invoke-FreshControl 'Main' @([Windows.Automation.ControlType]::Button) `
