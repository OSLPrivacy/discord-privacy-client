...
if(-not $ResultPath){
  ...
  $staged=Join-Path $root 'probe.ps1';$result=Join-Path $root 'result.json';Copy-Item -LiteralPath $PSCommandPath -Destination $staged -Force;Remove-Item -LiteralPath $result -Force -ErrorAction SilentlyContinue
  ...
  try{Start-ScheduledTask -TaskName $task;$deadline=[DateTime]::UtcNow.AddSeconds(60);while(-not(Test-Path -LiteralPath $result -PathType Leaf) -and [DateTime]::UtcNow -lt $deadline){Start-Sleep -Milliseconds 250};if(-not(Test-Path -LiteralPath $result -PathType Leaf)){throw 'interactive metadata probe timed out'};Get-Content -LiteralPath $result -Raw}finally{Unregister-ScheduledTask -TaskName $task -Confirm:$false -ErrorAction SilentlyContinue;Remove-Item -LiteralPath $staged,$result -Force -ErrorAction SilentlyContinue}
  exit 0
}
...
if($ResultPath){$temporary="$ResultPath.tmp";[IO.File]::WriteAllText($temporary,$json,[Text.UTF8Encoding]::new($false));Move-Item -LiteralPath $temporary -Destination $ResultPath -Force}else{$json}
