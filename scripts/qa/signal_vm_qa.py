#!/usr/bin/env python3
"""Fail-closed controller for a dedicated two-VM Signal Desktop QA lane."""
...
LEAVES = {
    "deploy": "osl-vm-deploy-preserve.ps1",
    "audit": "osl-vm-exact-build-audit.ps1",
    "arm": "osl-vm-signal-uia-arm.ps1",
    "poll": "osl-vm-signal-uia-poll.ps1",
}
CAPABILITIES = {
    "deploy": ("signal-deploy-preserve/v1", ("InvocationId", "ExeUri", "ExeSha256", "WebView2LoaderUri", "WebView2LoaderSha256", "SessionId", "WindowsUser", "ProfileRoot", "OslExePath", "SignalExePath", "SignalExeSha256", "SignalPublisherSubject")),
    "audit": ("signal-exact-build-audit/v1", ("SessionId", "WindowsUser", "ProfileRoot", "OslExePath", "OslExeSha256", "WebView2LoaderSha256", "SignalExePath", "SignalExeSha256", "SignalPublisherSubject")),
    "arm": ("signal-uia-arm/v1", ("InvocationId", "HarnessUri", "HarnessSha256", "Action", "SessionId", "WindowsUser", "ProfileRoot", "OslExePath", "OslExeSha256", "SignalExePath", "SignalExeSha256", "SignalPublisherSubject", "CaseId", "UploadUri")),
...
FORBIDDEN_KEYS = re.compile(
    ...
    r"qr|phone|message.?content)", re.I
)
...
def _reject_secrets(value: Any, at: str = "manifest") -> None:
    if isinstance(value, dict):
        for key, child in value.items():
            name = str(key)
            if name not in SAFE_REFERENCE_KEYS and FORBIDDEN_KEYS.search(name):
...
def validate_manifest(m: dict[str, Any]) -> None:
    _reject_secrets(m)
    if m.get("adapter") != "signal-desktop":
    ...
    required = ("resourceGroup", "vmName", "sessionId", "windowsUser", "profileRoot", "oslExePath", "signalExePath", "signalExeSha256", "signalPublisherSubject")
    for alias in ALIASES:
        ...
        if not isinstance(vm["sessionId"], int) or not 1 <= vm["sessionId"] <= 128:
        ...
        upload = vm.get("safeScreenshotUploadBaseUri")
        _query_free_https(upload, f"{alias}.safeScreenshotUploadBaseUri")
    leaf_dir = m.get("leafScriptDirectory")
    ...
    build = m.get("build")
    required_build = ("exeUri", "exeSha256", "webView2LoaderUri", "webView2LoaderSha256", "harnessUri", "harnessSha256")
...
def preflight(m: dict[str, Any]) -> dict[str, Path]:
    if not shutil.which("az"):
        raise QaError("missing tool: az (Azure CLI)")
    leaf_dir = Path(m["leafScriptDirectory"]).expanduser().resolve()
    ...
    for key, path in leaves.items():
        ...
        if f"# OSL-QA-Capability: {capability}" not in source:
            ...
            continue
        absent = [parameter for parameter in parameters if not re.search(rf"[$]{re.escape(parameter)}\b", source, re.I)]
        if absent:
...
def _az(vm: dict[str, Any], script: Path, parameters: dict[str, Any]) -> dict[str, Any]:
    command = ["az", "vm", "run-command", "invoke", "--only-show-errors", "-g", vm["resourceGroup"], "-n", vm["vmName"], "--command-id", "RunPowerShellScript", "--scripts", f"@{script}", "-o", "json", "--parameters"]
    ...
    for record in records:
        ...
        if not code.endswith("/succeeded") or ("/StdErr/" in code and message.strip()):
            # Guest error text can contain paths or provider-derived values.
            # Keep controller/receipts categorical and inspect only through a
            # separately bounded, content-safe diagnostic when needed.
...
def _parallel(stage: str, manifest: dict[str, Any], operation: Any) -> dict[str, Any]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        futures = {pool.submit(operation, alias, manifest["machines"][alias]): alias for alias in ALIASES}
...
def _common(vm: dict[str, Any], build: dict[str, Any]) -> dict[str, Any]:
    return {
        "SessionId": vm["sessionId"], "WindowsUser": vm["windowsUser"],
        "ProfileRoot": vm["profileRoot"], "OslExePath": vm["oslExePath"],
        "OslExeSha256": build["exeSha256"], "SignalExePath": vm["signalExePath"],
...
def stage_plan() -> list[dict[str, Any]]:
    ...
    stages.extend([
        {"id": "safe-chrome-screenshots", "mode": "parallel", "support": "implemented"},
    ])
    return stages
...
def run(manifest_path: Path, receipt_dir: Path) -> Path:
    ...
    receipt: dict[str, Any] = {
        "schemaVersion": 1, "runId": run_id, "adapter": "signal-desktop",
        "status": "running", "startedAt": dt.datetime.now(dt.timezone.utc).isoformat(),
        "machines": {alias: {"vmName": manifest["machines"][alias]["vmName"]} for alias in ALIASES},
        "stages": [],
    }
    ...
    def deploy(alias: str, vm: dict[str, Any], suffix: str) -> dict[str, Any]:
        return _az(vm, leaves["deploy"], {
        "SessionId": vm["sessionId"], "WindowsUser": vm["windowsUser"],
        "ProfileRoot": vm["profileRoot"], "OslExePath": vm["oslExePath"],
        "SignalExePath": vm["signalExePath"], "SignalExeSha256": vm["signalExeSha256"],
        "SignalPublisherSubject": vm["signalPublisherSubject"],
        "InvocationId": f"{run_id}-{alias[-1]}-{suffix}",
        "ExeUri": build["exeUri"], "ExeSha256": build["exeSha256"],
    ...
    def audit(_alias: str, vm: dict[str, Any]) -> dict[str, Any]:
        return _az(vm, leaves["audit"], {
        **_common(vm, build), "WebView2LoaderSha256": build["webView2LoaderSha256"],
        })
    ...
    # Redeploying the same immutable build verifies that the OS-sealed QA
    # identity and both application profiles survive process replacement.
    ...
    execute_stage("parallel-semantic-inventory", lambda alias, _vm: arm_and_poll(
        manifest_path, alias, "Inventory", "post-redeploy-inventory"
    ))
    ...
    receipt.update({
        "status": "ready", "terminal": True,
        "finishedAt": dt.datetime.now(dt.timezone.utc).isoformat(),
        "liveMatrix": {"status": "blockedUntilLiveAdapterReviewed", "caseCount": len(QA_CASES) * len(DIRECTIONS)},
...
def arm_and_poll(
    manifest_path: Path, alias: str, action: str, case_id: str,
    ...
    parameters = {
        **_common(vm, build), "InvocationId": invocation,
        "HarnessUri": build["harnessUri"], "HarnessSha256": build["harnessSha256"],
        "Action": action, "CaseId": case_id,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "validate":
            ...
            if not re.fullmatch(r"[A-Za-z0-9._-]{1,48}", args.case_id):
