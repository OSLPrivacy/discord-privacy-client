...
if [ "${1:-}" = "--self-test" ]; then
  instance_b_build_requires_distinct_identifier() {
    local tmp json log out rc repo fake_bin stage temp_root dll cargo_args cargo_cwd tauri_config
    tmp="$(mktemp -d)"
...
PY
    ...
    tauri_config="$tmp/tauri-config.json"
...
SH
    ...
    REPO="$repo" JSON_OUT="$json" LOG="$log" STAGE="$stage" \
      ...
      FAKE_TAURI_CONFIG="$tauri_config" \
...
PY
    python3 - "$tauri_config" <<'PY' || return 1
import json, sys
